# Contributing to Artee VPN

Thanks for your interest in contributing to Artee VPN. 
...
If you haven't already, join our slack workspace [here](https://docs.Artee VPN.io/slack-url), we would love to discuss topics that need community contribution and enhancements to existing features.

## Contents

- [Contributing to Artee VPN](#contributing-to-Artee VPN)
    - [Contents](#contents)
    - [Code of conduct](#code-of-conduct)
    - [Discuss changes with the Artee VPN team first](#discuss-changes-with-the-Artee VPN-team-first)
    - [Directory structure](#directory-structure)
    - [Development setup](#development-setup)
        - [Requirements](#requirements)
        - [Local Artee VPN setup](#local-Artee VPN-setup)
...
unacceptable behavior to community@Artee VPN.io.

## Discuss changes with the Artee VPN team first

Changes to the **public API**, **gRPC protocols**, **functionality behavior**, **CLI / service flags**, or **new features** should be discussed with the Artee VPN team before you start the work. These surfaces are part of Artee VPN's contract with operators, self-hosters, and downstream integrators, and changes to them have compatibility, security, and release-planning implications that benefit from an early conversation.

Open an issue or reach out on [Slack](https://docs.Artee VPN.io/slack-url) to talk through what you have in mind. We'll help shape the change, flag any constraints we know about, and confirm the direction so the PR review can focus on implementation rather than design.

Typical bug fixes, internal refactors, documentation updates, and tests do not need pre-discussion — open the PR directly.

## Directory structure

The Artee VPN project monorepo is organized to maintain most of its individual dependencies code within their directories, except for a few auxiliary or shared packages.

The most important directories are:

- [/.github](/.github) - Github actions workflow files and issue templates
- [/client](/client) - Artee VPN agent code
- [/client/cmd](/client/cmd) - Artee VPN agent cli code
- [/client/internal](/client/internal) - Artee VPN agent business logic code
- [/client/proto](/client/proto) - Artee VPN agent daemon GRPC proto files
- [/client/server](/client/server) - Artee VPN agent daemon code for background execution
- [/client/ui](/client/ui) - Artee VPN agent UI code
...
### Local Artee VPN setup

> **IMPORTANT**: All the steps below have to get executed at least once to get the development setup up and running!

Now that everything Artee VPN requires to run is installed, the actual Artee VPN code can be
checked out and set up:

1. [Fork](https://guides.github.com/activities/forking/#fork) the Artee VPN repository

2. Clone your forked repository

   ```
   git clone https://github.com/<your_github_username>/Artee VPN.git
   ```
...
4. Add the original Artee VPN repository as `upstream` to your forked repository

   ```
   git remote add upstream https://github.com/Artee VPNio/Artee VPN.git
   ```
...
If you prefer using a dev container for development, Artee VPN now includes support for dev containers. 
Dev containers provide a consistent and isolated development environment, making it easier for contributors to get started quickly. Follow the steps below to set up Artee VPN in a dev container.
...
Clone the repository following previous [Local Artee VPN setup](#local-Artee VPN-setup).
...
Open GoLand and select `"File" > "Open"` to open the Artee VPN project folder.
...
To start Artee VPN the client in the foreground:
...
To start Artee VPN's signal, execute:
...
To start Artee VPN the signal service:
...
#### Management service
> You may need to generate a configuration file for management. Follow steps 2 to 5 from our [self-hosting guide](https://Artee VPN.io/docs/getting-started/self-hosting).

To start Artee VPN's management, execute:
...
To start Artee VPN the management service:

```
./management management --log-level debug --log-file console --config ./management.json
```

#### Windows Artee VPN Installer
Create dist directory
```shell
mkdir -p dist/Artee VPN_windows_amd64
```

UI client
```shell
CC=x86_64-w64-mingw32-gcc CGO_ENABLED=1 GOOS=windows GOARCH=amd64 go build -o Artee VPN-ui.exe -ldflags "-s -w -H windowsgui" ./client/ui
mv Artee VPN-ui.exe ./dist/Artee VPN_windows_amd64/
```

Client
```shell
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o Artee VPN.exe ./client/
mv Artee VPN.exe ./dist/Artee VPN_windows_amd64/
```
> Windows clients have a Wireguard driver requirement. You can download the wintun driver from https://www.wintun.net/builds/wintun-0.14.1.zip, after decompressing, you can copy the file `windtun\bin\ARCH\wintun.dll` to `./dist/Artee VPN_windows_amd64/`.
...
The installer `Artee VPN-installer.exe` will be created in root directory.
...
Artee VPN project is composed of 3 main repositories:
- Artee VPN: This repository, which contains the code for the agents and control plane services.
- Dashboard: https://github.com/Artee VPNio/dashboard, contains the Administration UI for the management service
- Documentations: https://github.com/Artee VPNio/docs, contains the documentation from https://Artee VPN.io/docs
