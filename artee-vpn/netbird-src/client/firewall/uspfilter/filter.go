...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/firewall/uspfilter/common"
	"github.com/Artee VPNio/Artee VPN/client/firewall/uspfilter/conntrack"
	"github.com/Artee VPNio/Artee VPN/client/firewall/uspfilter/forwarder"
	nblog "github.com/Artee VPNio/Artee VPN/client/firewall/uspfilter/log"
...
const (
	...
	// EnvEnableLocalForwarding enables forwarding of local traffic to the native stack for internal (non-Artee VPN) interfaces.
...
func (m *Manager) shouldForward(d *decoder, dstIP netip.Addr) bool {
	...
	// traffic to our other local interfaces (not Artee VPN IP) - always forward
	addr := m.wgIface.Address()
	if dstIP != addr.IP && (!addr.IPv6.IsValid() || dstIP != addr.IPv6) {
		return true
	}

	// traffic to our Artee VPN IP, not netstack mode - send to netstack listeners
	return false
}
