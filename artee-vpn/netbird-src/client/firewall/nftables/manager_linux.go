...
const (
	// tableNameArtee VPN is the default name of the table that is used for filtering by the Artee VPN client
	tableNameArtee VPN = "Artee VPN"
...
// a known state (our Artee VPN table plus a few static rules). This allows for easy
...
func (m *Manager) rollbackInit() {
	...
	if err := m.cleanupArtee VPNTables(); err != nil {
...
//
// TODO: In USP mode this only adds ACCEPT to the Artee VPN table's own chains,
...
// addExternalChainsRules does) for both the Artee VPN table family and inet tables.
// The Artee VPN table itself is fine (routing chains already exist there), but
// non-Artee VPN tables with INPUT/FORWARD hooks can still DROP our WG traffic.
func (m *Manager) AllowArtee VPN() error {
	...
	if err := m.rConn.Flush(); err != nil {
		return fmt.Errorf("flush allow input Artee VPN rules: %w", err)
	}
...
func (m *Manager) Close(stateManager *statemanager.Manager) error {
	...
	if err := m.cleanupArtee VPNTables(); err != nil {
		merr = multierror.Append(merr, fmt.Errorf("cleanup Artee VPN tables: %v", err))
	}
...
func (m *Manager) cleanupArtee VPNTables() error {
...
const (
	chainNameRawOutput     = "Artee VPN-raw-out"
	chainNameRawPrerouting = "Artee VPN-raw-pre"
)
