...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/firewall/test"
	"github.com/Artee VPNio/Artee VPN/client/iface"
	"github.com/Artee VPNio/Artee VPN/client/internal/acl/id"
)
...
func createWorkTableIPv6() (*nftables.Table, error) {
	...
	for _, t := range tables {
		if t.Name == tableNameArtee VPN {
			sConn.DelTable(t)
		}
	}

	table := sConn.AddTable(&nftables.Table{Name: tableNameArtee VPN, Family: nftables.TableFamilyIPv6})
	err = sConn.Flush()
	return table, err
}
...
func createWorkTable() (*nftables.Table, error) {
	...
	table := sConn.AddTable(&nftables.Table{Name: tableNameArtee VPN, Family: nftables.TableFamilyIPv4})
	err = sConn.Flush()
