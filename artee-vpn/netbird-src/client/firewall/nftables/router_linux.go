...
import (
	...
	nbid "github.com/Artee VPNio/Artee VPN/client/internal/acl/id"
	"github.com/Artee VPNio/Artee VPN/client/internal/routemanager/ipfwdstate"
...
const (
	...
	chainNameRoutingFw     = "Artee VPN-rt-fwd"
	chainNameRoutingNat    = "Artee VPN-rt-postrouting"
	chainNameRoutingRdr    = "Artee VPN-rt-redirect"
	chainNameNATOutput     = "Artee VPN-nat-output"
	chainNameForward       = "FORWARD"
	chainNameMangleForward = "Artee VPN-mangle-forward"
...
func (r *router) AddNatRule(pair firewall.RouterPair) error {
	...
	if r.legacyManagement {
		log.Warnf("This peer is connected to a Artee VPN Management service with an older version. Allowing all traffic for %s", pair.Destination)
...
// acceptExternalChainsRules adds accept rules to external chains (non-Artee VPN, non-iptables tables).
...
func (r *router) insertForwardAcceptRules(chain *nftables.Chain, intf []byte) {
	existing, err := r.existingArtee VPNRulesInChain(chain)
	if err != nil {
...
// existingArtee VPNRulesInChain returns the set of Artee VPN-owned UserData tags present in a chain; callers must bail on error since InsertRule is additive.
func (r *router) existingArtee VPNRulesInChain(chain *nftables.Chain) (map[string]bool, error) {
	...
	for _, rule := range rules {
		if !isArtee VPNAcceptRuleTag(rule.UserData) {
			continue
		}
...
func isArtee VPNAcceptRuleTag(userData []byte) bool {
...
// findExternalChains scans for chains from non-Artee VPN tables that have FORWARD or INPUT hooks.
...
// AddInboundDNAT adds an inbound DNAT rule redirecting traffic from Artee VPN peers to local services.
