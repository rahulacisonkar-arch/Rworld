...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/internal/auth"
	"github.com/Artee VPNio/Artee VPN/client/internal/expose"
	"github.com/Artee VPNio/Artee VPN/client/internal/profilemanager"
	sleephandler "github.com/Artee VPNio/Artee VPN/client/internal/sleep/handler"
...
func (s *Server) SetConfig(callerCtx context.Context, msg *proto.SetConfigRequest) (*proto.SetConfigResponse, error) {
	...
	// every `Artee VPN up` (setupSetConfigReq in cmd/up.go), so a plain
	// `Artee VPN up` would otherwise always trip the gate and surface a
...
func (s *Server) Login(callerCtx context.Context, msg *proto.LoginRequest) (*proto.LoginResponse, error) {
	...
	// command `Artee VPN up --management-url=X` (which falls through to
...
func (s *Server) Status(
	...
	statusResponse := proto.StatusResponse{Status: string(status), DaemonVersion: version.Artee VPNVersion()}
...
// ExposeService exposes a local port via the Artee VPN reverse proxy.
func (s *Server) ExposeService(req *proto.ExposeServiceRequest, srv proto.DaemonService_ExposeServiceServer) error {
	s.mutex.Lock()
	if !s.clientRunning {
		s.mutex.Unlock()
		return gstatus.Errorf(codes.FailedPrecondition, "client is not running, run 'Artee VPN up' first")
	}
	connectClient := s.connectClient
	s.mutex.Unlock()
...
// to inform the user that the Artee VPN connection session has expired.
func sendTerminalNotification() error {
	message := "Artee VPN connection session expired\n\nPlease re-authenticate to connect to the network."
