...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/internal/daemonaddr"
	"github.com/Artee VPNio/Artee VPN/client/internal/profilemanager"
	"github.com/Artee VPNio/Artee VPN/client/proto"
	nbssh "github.com/Artee VPNio/Artee VPN/client/ssh"
	"github.com/Artee VPNio/Artee VPN/client/ssh/detection"
	"github.com/Artee VPNio/Artee VPN/util"
	"github.com/Artee VPNio/Artee VPN/util/netrelay"
)

const (
	// DefaultDaemonAddr is the default address for the Artee VPN daemon
	DefaultDaemonAddr = "unix:///var/run/Artee VPN.sock"
	// DefaultDaemonAddrWindows is the default address for the Artee VPN daemon on Windows
...
// requestJWTToken requests a JWT token from the Artee VPN daemon
...
// verifyHostKeyViaDaemon verifies SSH host key by querying the Artee VPN daemon
...
func connectToDaemon(daemonAddr string) (*grpc.ClientConn, error) {
	...
	if err != nil {
		log.Debugf("failed to create gRPC client for Artee VPN daemon at %s: %v", daemonAddr, err)
		return nil, fmt.Errorf("failed to connect to Artee VPN daemon: %w", err)
	}
...
func getKnownHostsFiles() []string {
	...
	// Artee VPN managed known_hosts files
	if runtime.GOOS == "windows" {
		programData := os.Getenv("PROGRAMDATA")
		if programData == "" {
			programData = `C:\ProgramData`
		}
		Artee VPNKnownHosts := filepath.Join(programData, "ssh", "ssh_known_hosts.d", "99-Artee VPN")
		files = append(files, Artee VPNKnownHosts)
	} else {
		files = append(files, "/etc/ssh/ssh_known_hosts.d/99-Artee VPN")
