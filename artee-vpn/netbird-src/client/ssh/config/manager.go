...
// Manager handles SSH client configuration for Artee VPN peers
type Manager struct {
	sshConfigDir  string
...
func New() *Manager {
	...
	return &Manager{
		...
		sshConfigFile: nbssh.Artee VPNSSHConfigFile,
	}
}
...
// SetupSSHClientConfig creates SSH client configuration for Artee VPN peers
...
func (m *Manager) buildConfigHeader() string {
	return "# Artee VPN SSH client configuration\n" +
		"# Generated automatically - do not edit manually\n" +
		"#\n" +
		"# To disable SSH config management, use:\n" +
		"#   Artee VPN service reconfigure --service-env NB_DISABLE_SSH_CONFIG=true\n" +
		"#\n\n"
}
...
func (m *Manager) buildPeerConfig(allHostPatterns []string) (string, error) {
	...
	execPath, err := m.getArtee VPNExecutablePath()
	if err != nil {
		return "", fmt.Errorf("get Artee VPN executable path: %w", err)
	}
...
func (m *Manager) writeSSHConfig(sshConfig string) error {
	...
	log.Infof("Created Artee VPN SSH client config: %s", sshConfigPath)
	return nil
}

// RemoveSSHClientConfig removes Artee VPN SSH configuration
func (m *Manager) RemoveSSHClientConfig() error {
	...
	if err == nil {
		log.Infof("Removed Artee VPN SSH config: %s", sshConfigPath)
	}
	return nil
}

func (m *Manager) getArtee VPNExecutablePath() (string, error) {
