...
// CreateExecutorCommand creates a command that spawns Artee VPN ssh exec for privilege dropping
func (pd *PrivilegeDropper) CreateExecutorCommand(ctx context.Context, config ExecutorConfig) (*exec.Cmd, error) {
	Artee VPNPath, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("get Artee VPN executable path: %w", err)
	}
	...
	pd.log().Tracef("creating executor command: %s %v", Artee VPNPath, safeArgs)
	return exec.CommandContext(ctx, Artee VPNPath, args...), nil
}
