...
func TestCheckPrivileges_ComprehensiveMatrix(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			os:                        "windows",
			euid:                      1000,
			currentUser:               createTestUser("Artee VPN\\WIN2K19-C2$", "S-1-5-18", "S-1-5-18", "C:\\Windows\\System32"),
			requestedUsername:         "root",
			featureSupportsUserSwitch: true,
			allowRoot:                 false,
			expectedAllowed:           false, // Root not allowed
...
func TestUsedFallback_MeansNoPrivilegeDropping(t *testing.T) {
	...
	getCurrentUser = func() (*user.User, error) {
		return createTestUser("Artee VPN", "1000", "1000", "/var/lib/Artee VPN"), nil
	}
	...
	assert.Equal(t, "Artee VPN", result.User.Username, "Should return current user")
