...
func (s *Server) createSftpCommand(targetUser *user.User, sess ssh.Session) (*exec.Cmd, windows.Token, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("get Artee VPN executable path: %w", err)
	}
	...
	cmd, primaryToken, err := pd.createProcessWithToken(sess.Context(), windows.Token(token), Artee VPNPath, append([]string{Artee VPNPath}, args...), targetUser.HomeDir)
	if err != nil {
