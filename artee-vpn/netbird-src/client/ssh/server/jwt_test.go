...
import (
	...
	sshauth "github.com/Artee VPNio/Artee VPN/client/ssh/auth"
	"github.com/Artee VPNio/Artee VPN/client/ssh/client"
	"github.com/Artee VPNio/Artee VPN/client/ssh/detection"
	"github.com/Artee VPNio/Artee VPN/client/ssh/testutil"
	nbjwt "github.com/Artee VPNio/Artee VPN/shared/auth/jwt"
	sshuserhash "github.com/Artee VPNio/Artee VPN/shared/sshauth"
)
...
func TestJWTEnforcement(t *testing.T) {
	...
	t.Run("allows_when_disabled", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, detection.ServerTypeArtee VPNNoJWT, serverType)
		assert.False(t, serverType.RequiresJWT())

		client, err := connectWithArtee VPNClient(t, hostNoJWT, portNoJWT)
		require.NoError(t, err)
		defer client.Close()
	})
...
// connectWithArtee VPNClient connects to SSH server using Artee VPN's SSH client
func connectWithArtee VPNClient(t *testing.T, host string, port int) (*client.Client, error) {
	t.Helper()
...
func TestJWTDetection(t *testing.T) {
	...
	serverType, err := detection.DetectSSHServerType(context.Background(), dialer, host, port)
	require.NoError(t, err)
	assert.Equal(t, detection.ServerTypeArtee VPNJWT, serverType)
