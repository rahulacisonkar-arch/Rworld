...
import (
	...
	"github.com/Artee VPNio/Artee VPN/shared/auth/jwt"
	"github.com/Artee VPNio/Artee VPN/util/netrelay"
	"github.com/Artee VPNio/Artee VPN/version"
)

// DefaultSSHPort is the default SSH port of the Artee VPN's embedded SSH server
...
func (s *Server) connectionValidator(_ ssh.Context, conn net.Conn) net.Conn {
	...
	if !inV4 && !inV6 {
		log.Warnf("SSH connection rejected from non-Artee VPN IP %s", remoteIP)
		return nil
	}

	log.Infof("SSH connection from Artee VPN peer %s allowed", tcpAddr)
	return &trackedConn{
		Conn:       conn,
		server:     s,
...
func (s *Server) createSSHServer(addr net.Addr) (*ssh.Server, error) {
	...
	serverVersion := fmt.Sprintf("%s-%s", detection.ServerIdentifier, version.Artee VPNVersion())
	if s.jwtEnabled {
