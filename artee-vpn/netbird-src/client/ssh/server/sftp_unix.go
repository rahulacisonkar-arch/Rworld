...
// createSftpExecutorCommand creates a command that spawns Artee VPN ssh sftp for privilege dropping
func (s *Server) createSftpExecutorCommand(sess ssh.Session, uid, gid uint32, groups []uint32, workingDir string) (*exec.Cmd, error) {
	...
	log.Tracef("creating SFTP executor command: %s %v", Artee VPNPath, args)
	return exec.CommandContext(sess.Context(), Artee VPNPath, args...), nil
}
