...
Artee VPN_BIN="${Artee VPN_BIN:-"Artee VPN"}"
export NB_LOG_FILE="${NB_LOG_FILE:-"console,/var/log/Artee VPN/client.log"}"
service_pids=()
...
on_exit() {
  info "Shutting down Artee VPN daemon..."
...
wait_for_daemon_startup() {
  ...
  while [[ "${SECONDS}" -lt "${deadline}" ]]; do
    if "${Artee VPN_BIN}" status --check live 2>/dev/null; then
      return
    fi
    sleep 1
  done
...
main() {
  ...
  "${Artee VPN_BIN}" service run &
  service_pids+=("$!")
  info "registered new service process 'Artee VPN service run', currently running: ${service_pids[@]@Q}"
