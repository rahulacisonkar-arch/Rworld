...
// Conn wraps a WebSocket connection over a Artee VPN TCP connection.
type Conn struct {
	conn      net.Conn
	mu        sync.Mutex
	closed    chan struct{}
...
// Dial establishes a WebSocket connection to the given URL through the Artee VPN network.
// Optional protocols are sent via the Sec-WebSocket-Protocol header.
func Dial(ctx context.Context, client *Artee VPN.Client, rawURL string, protocols []string) (*Conn, error) {
	d := ws.Dialer{
		NetDial:   client.Dial,
		Protocols: protocols,
	}
