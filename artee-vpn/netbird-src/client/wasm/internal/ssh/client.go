...
func NewClient(nbClient *Artee VPN.Client) *Client {
	return &Client{
		nbClient: nbClient,
	}
}

// Connect establishes an SSH connection through Artee VPN network.
...
func (c *Client) getAuthMethods(jwtToken string) ([]ssh.AuthMethod, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("get Artee VPN config: %w", err)
	}

	if nbConfig.SSHKey == "" {
		return nil, fmt.Errorf("no Artee VPN SSH key available")
	}

	signer, err := ssh.ParsePrivateKey([]byte(nbConfig.SSHKey))
	if err != nil {
		return nil, fmt.Errorf("parse Artee VPN SSH private key: %w", err)
	}
