...
type Handle struct {
	cs      *Artee VPN.CaptureSession
	stopFn  js.Func
	stopped bool
}

// Stop ends the capture and returns stats.
func (h *Handle) Stop() Artee VPN.CaptureStats {
	if h.stopped {
		return h.cs.Stats()
	}
	h.stopped = true
...
func statsToJS(s Artee VPN.CaptureStats) js.Value {
...
func Start(client *Artee VPN.Client, jsOpts js.Value) (js.Value, error) {
	filter, verbose, ascii := parseOpts(jsOpts)

	cb := &jsCallbackWriter{}

	cs, err := client.StartCapture(Artee VPN.CaptureOptions{
		TextOutput: cb,
		Filter:     filter,
		Verbose:    verbose,
		ASCII:      ascii,
...
func StartConsole(client *Artee VPN.Client, jsOpts js.Value) (*Handle, error) {
	filter, verbose, ascii := parseOpts(jsOpts)
