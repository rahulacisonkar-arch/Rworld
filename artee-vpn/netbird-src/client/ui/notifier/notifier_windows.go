...
const (
	...
	// shortcut by the MSI (see client/Artee VPN.wxs); otherwise Windows
	// groups toasts under a separate, unbranded entry.
	appID = "Artee VPN"

	// appGUID identifies the COM activation callback class. Generated once
	// for Artee VPN; do not change without coordinating an installer bump,
...
func resolveIcon() string {
	...
	candidate := filepath.Join(filepath.Dir(exe), "Artee VPN.png")
