...
import (
	...
	uptypes "github.com/Artee VPNio/Artee VPN/upload-server/types"
...
func (s *serviceClient) showDebugUI() {
	w := s.app.NewWindow("Artee VPN Debug")
	...
	content := container.NewVBox(
		widget.NewLabel("Create a debug bundle to help troubleshoot issues with Artee VPN"),
		widget.NewLabel(""),
		anonymizeCheck, systemInfoCheck, captureCheck,
...
func (s *serviceClient) buildDurationSection() (*fyne.Container, *widget.Check, *widget.Entry, *widget.Label) {
	...
	noteLabel := widget.NewLabel("Note: Artee VPN will be brought up and down during collection")
