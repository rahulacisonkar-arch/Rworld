...
func (s *serviceClient) getArtee VPNImage(name string, content []byte) *canvas.Image {
...
func (s *serviceClient) applyQuickActionsUiState(
	...
	fyne.DoAndWait(func() {
		...
		// the second position in the content's object array is the Artee VPN logo.
		if logo != nil {
...
// showQuickActionsUI displays a simple window with the Artee VPN logo and a connection toggle button.
func (s *serviceClient) showQuickActionsUI() {
	s.wQuickActions = s.app.NewWindow("Artee VPN")
	...
	connectedImage := s.getArtee VPNImage("Artee VPN.png", iconAbout)
	disconnectedImage := s.getArtee VPNImage("Artee VPN-disconnected.png", iconAboutDisconnected)
	...
	hintLabelText := fmt.Sprintf("You can always access Artee VPN from your %s.", getSystemTrayName())
