...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/mdm"
	"github.com/Artee VPNio/Artee VPN/client/proto"
	"github.com/Artee VPNio/Artee VPN/client/ui/desktop"
	"github.com/Artee VPNio/Artee VPN/client/ui/event"
	"github.com/Artee VPNio/Artee VPN/client/ui/notifier"
	"github.com/Artee VPNio/Artee VPN/client/ui/process"
...
func main() {
	...
	a := app.NewWithID("Artee VPN")
	a.SetIcon(fyne.NewStaticResource("Artee VPN", iconDisconnected))
...
func parseFlags() *cliFlags {
	...
	defaultDaemonAddr := "unix:///var/run/Artee VPN.sock"
	...
	flag.BoolVar(&flags.saveLogsInFile, "use-log-file", false, fmt.Sprintf("save logs in a file: %s/Artee VPN-ui-PID.log", os.TempDir()))
...
func initLogFile() (string, error) {
	logFile := path.Join(os.TempDir(), fmt.Sprintf("Artee VPN-ui-%d.log", os.Getpid()))
...
//go:embed assets/Artee VPN-systemtray-connected-macos.png
var iconConnectedMacOS []byte

//go:embed assets/Artee VPN-systemtray-disconnected-macos.png
var iconDisconnectedMacOS []byte

//go:embed assets/Artee VPN-systemtray-update-disconnected-macos.png
var iconUpdateDisconnectedMacOS []byte

//go:embed assets/Artee VPN-systemtray-update-connected-macos.png
var iconUpdateConnectedMacOS []byte

//go:embed assets/Artee VPN-systemtray-connecting-macos.png
var iconConnectingMacOS []byte

//go:embed assets/Artee VPN-systemtray-error-macos.png
...
func (s *serviceClient) showSettingsUI() {
	...
	s.wSettings = s.app.NewWindow("Artee VPN Settings")
	...
	s.sNetworkMonitor = widget.NewCheck("Restarts Artee VPN when the network changes", nil)
...
func (s *serviceClient) updateStatus() error {
	...
	err = backoff.Retry(func() error {
		...
		case status.Status == string(internal.StatusConnected) && !s.connected:
			...
			systray.SetTooltip("Artee VPN (Connected)")
...
func (s *serviceClient) setDisconnectedStatus() {
	...
	systray.SetTooltip("Artee VPN (Disconnected)")
...
func (s *serviceClient) setConnectingStatus() {
	s.connected = false
	systray.SetTemplateIcon(iconConnectingMacOS, s.icConnecting)
	systray.SetTooltip("Artee VPN (Connecting)")
...
func (s *serviceClient) onTrayReady() {
	...
	versionString := normalizedVersion(version.Artee VPNVersion())
...
func (s *serviceClient) showLoginURL() context.CancelFunc {
	...
	resIcon := fyne.NewStaticResource("Artee VPN.png", iconAbout)

	if s.wLoginURL == nil {
		s.wLoginURL = s.app.NewWindow("Artee VPN Session Expired")
	...
	label := widget.NewLabel("Your Artee VPN session has expired.\nPlease re-authenticate to continue using Artee VPN.")
