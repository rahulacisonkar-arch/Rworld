...
/*
 Most of this code was copied from https://github.com/pion/ice and modified to fulfill Artee VPN's requirements.
*/
...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/iface/bufsize"
...
func (u *UDPConn) WriteTo(b []byte, addr net.Addr) (int, error) {
	...
	if (u.address.Network.IsValid() && u.address.Network.Contains(dst)) || (u.address.IPv6Net.IsValid() && u.address.IPv6Net.Contains(dst)) {
		log.Warnf("address %s is part of the Artee VPN network %s, refusing to write", addr, u.address)
		return 0, fmt.Errorf("address %s is part of the Artee VPN network %s, refusing to write", addr, u.address)
	}
	return u.PacketConn.WriteTo(b, addr)
}
