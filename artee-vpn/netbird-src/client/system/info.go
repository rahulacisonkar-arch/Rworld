...
// extractUserAgent extracts Artee VPN's agent (client) name and version from the outgoing context
func extractUserAgent(ctx context.Context) string {
	...
	if hasMeta {
		...
		if ok {
			...
			if strings.HasPrefix(nbAgent, "Artee VPN") {
				return nbAgent
			}
			return ""
		}
