...
var profileCmd = &cobra.Command{
	Use:   "profile",
	Short: "Manage Artee VPN client profiles",
...
var profileListCmd = &cobra.Command{
	...
	Long:    `List all available profiles in the Artee VPN client.`,
	Aliases: []string{"ls"},
...
func addProfileFunc(cmd *cobra.Command, args []string) error {
	...
	if dupCount > 1 {
		...
		cmd.Println("Use `Artee VPN profile list --show-id` to disambiguate later.")
	}
...
func wrapAmbiguityError(err error, handle string) error {
	...
	case codes.InvalidArgument:
		...
		if strings.Contains(msg, "ambiguous") {
			return errors.New(msg + "\nRun `Artee VPN profile list --show-id` to see IDs, then select by ID prefix:\n  Artee VPN profile select|remove <id-prefix>")
		}
	case codes.NotFound:
...
// creation, shared by `Artee VPN profile add` and the `Artee VPN up --profile
