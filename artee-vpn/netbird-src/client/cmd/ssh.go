...
import (
	...
	sshclient "github.com/Artee VPNio/Artee VPN/client/ssh/client"
	"github.com/Artee VPNio/Artee VPN/client/ssh/detection"
	sshproxy "github.com/Artee VPNio/Artee VPN/client/ssh/proxy"
	sshserver "github.com/Artee VPNio/Artee VPN/client/ssh/server"
...
var sshCmd = &cobra.Command{
	...
	Short: "Connect to a Artee VPN peer via SSH",
	Long: `Connect to a Artee VPN peer using SSH with support for port forwarding.
...
Examples:
  Artee VPN ssh peer-hostname
  Artee VPN ssh root@peer-hostname
  Artee VPN ssh --login root peer-hostname
  Artee VPN ssh peer-hostname ls -la
  Artee VPN ssh peer-hostname whoami
  Artee VPN ssh -t peer-hostname tmux                  # Force PTY for tmux/screen
  Artee VPN ssh -t peer-hostname sudo -i               # Force PTY for interactive sudo
  Artee VPN ssh -L 8080:localhost:80 peer-hostname     # Local port forwarding
  Artee VPN ssh -R 9090:localhost:3000 peer-hostname   # Remote port forwarding
  Artee VPN ssh -L "*:8080:localhost:80" peer-hostname # Bind to all interfaces
  Artee VPN ssh -L 8080:/tmp/socket peer-hostname      # Unix socket forwarding`,
...
func createSSHFlagSet() (*flag.FlagSet, *sshFlags) {
	...
	fs.StringVar(&flags.ConfigPath, "c", defaultConfigPath, "Artee VPN config file location")
	fs.StringVar(&flags.ConfigPath, "config", defaultConfigPath, "Artee VPN config file location")
	fs.StringVar(&flags.LogLevel, "l", defaultLogLevel, "sets Artee VPN log level")
	fs.StringVar(&flags.LogLevel, "log-level", defaultLogLevel, "sets Artee VPN log level")
...
func runSSH(ctx context.Context, addr string, cmd *cobra.Command) error {
	...
	if err != nil {
		...
		cmd.Printf("  1. Check peer connectivity: Artee VPN status -d\n")
...
var sshDetectCmd = &cobra.Command{
	...
	Short:  "Detect if a host is running Artee VPN SSH",
	Long:   "Internal command used by SSH Match exec to detect Artee VPN SSH servers. Exit codes: 0=JWT, 1=no-JWT, 2=regular SSH",
	Hidden: true,
	Args:   cobra.ExactArgs(2),
	RunE:   sshDetectFn,
}
