...
var installCmd = &cobra.Command{
	Use:   "install",
	Short: "Install Artee VPN service",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		cmd.Println("Artee VPN service has been installed")
		return nil
	},
}
...
var uninstallCmd = &cobra.Command{
	Use:   "uninstall",
	Short: "uninstalls Artee VPN service from system",
	RunE: func(cmd *cobra.Command, args []string) error {
		if err := setupServiceCommand(cmd); err != nil {
			return err
		}
		...
		cmd.Println("Artee VPN service has been uninstalled")
		return nil
	},
}
...
var reconfigureCmd = &cobra.Command{
	Use:   "reconfigure",
	Short: "reconfigures Artee VPN service with new settings",
	Long: `Reconfigures the Artee VPN service with new settings without manual uninstall/install.
This command will temporarily stop the service, update its configuration, and restart it if it was running.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if wasRunning {
			cmd.Println("Stopping Artee VPN service...")
		...
		if wasRunning {
			cmd.Println("Starting Artee VPN service...")
			if err := s.Start(); err != nil {
				return fmt.Errorf("start service after reconfigure: %w", err)
			}
			cmd.Println("Artee VPN service has been reconfigured and started")
		} else {
			cmd.Println("Artee VPN service has been reconfigured")
		}
...
const (
	...
	networkdConfFile    = "/etc/systemd/networkd.conf.d/99-Artee VPN.conf"
	networkdConfContent = `# Created by Artee VPN to prevent systemd-networkd from removing
# routes and policy rules managed by Artee VPN.
...
// systemd-networkd from removing Artee VPN's routes and policy rules.
...
// cleanupSystemdNetworkd removes the Artee VPN systemd-networkd configuration file.
