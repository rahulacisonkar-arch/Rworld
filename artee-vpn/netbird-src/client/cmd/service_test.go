...
func TestServiceLifecycle(t *testing.T) {
	...
	serviceName = "Artee VPNtest" + fmt.Sprintf("%d", time.Now().Unix())
	defer func() {
		serviceName = originalServiceName
	}()

	tempDir := t.TempDir()
	configPath = fmt.Sprintf("%s/Artee VPN-test-config.json", tempDir)
	logLevel = "info"
	daemonAddr = fmt.Sprintf("unix://%s/Artee VPN-test.sock", tempDir)
