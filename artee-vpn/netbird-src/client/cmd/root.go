...
import (
	...
	daddr "github.com/Artee VPNio/Artee VPN/client/internal/daemonaddr"
...
func init() {
	defaultConfigPathDir = "/etc/Artee VPN/"
	defaultLogFileDir = "/var/log/Artee VPN/"
	...
	switch runtime.GOOS {
	case "windows":
		defaultConfigPathDir = os.Getenv("PROGRAMDATA") + "\\Artee VPN\\"
		defaultLogFileDir = os.Getenv("PROGRAMDATA") + "\\Artee VPN\\"

		oldDefaultConfigPathDir = os.Getenv("PROGRAMDATA") + "\\Wiretrustee\\"
		oldDefaultLogFileDir = os.Getenv("PROGRAMDATA") + "\\Wiretrustee\\"
	case "freebsd":
		defaultConfigPathDir = "/var/db/Artee VPN/"
	}
	...
	rootCmd.PersistentFlags().StringVarP(&logLevel, "log-level", "l", "info", "sets Artee VPN log level")
	rootCmd.PersistentFlags().StringSliceVar(&logFiles, "log-file", []string{defaultLogFile}, "sets Artee VPN log paths written to simultaneously. If `console` is specified the log will be output to stdout. If `syslog` is specified the log will be sent to syslog daemon. You can pass the flag multiple times or separate entries by `,` character")
	...
	rootCmd.PersistentFlags().BoolVarP(&anonymizeFlag, "anonymize", "A", false, "anonymize IP addresses and non-Artee VPN.io domains in logs and status output")
	...
	upCmd.PersistentFlags().StringVar(&customDNSAddress, dnsResolverAddress, "",
		`Sets a custom address for Artee VPN's local DNS resolver. `+
...
func handleRebrand(cmd *cobra.Command) error {
	...
	if slices.Contains(logFiles, defaultLogFile) {
		if migrateToArtee VPN(oldDefaultLogFile, defaultLogFile) {
	...
	if migrateToArtee VPN(oldDefaultConfigPath, defaultConfigPath) {
...
func migrateToArtee VPN(oldPath, newPath string) bool {
...
func getClient(cmd *cobra.Command) (*grpc.ClientConn, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to connect to daemon error: %v\n"+
			...
			"\nArtee VPN service install \nArtee VPN service start\n", err)
	}
