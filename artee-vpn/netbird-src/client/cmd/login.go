...
func init() {
	...
	loginCmd.PersistentFlags().StringVarP(&configPath, "config", "c", "", "(DEPRECATED) Artee VPN config file location")
}

var loginCmd = &cobra.Command{
	Use:   "login",
	Short: "Log in to the Artee VPN network",
	Long:  "Log in to the Artee VPN network using a setup key or SSO",
...
func getActiveProfile(ctx context.Context, pm *profilemanager.ProfileManager, profileName string, username string) (*profilemanager.Profile, error) {
	...
	if activeProf == nil {
		return nil, fmt.Errorf("active profile not found, please run 'Artee VPN profile create' first")
	}
	...
	if !noBrowser {
		if err := util.OpenBrowser(verificationURIComplete); err != nil {
			cmd.Println("\nAlternatively, you may want to use a setup key, see:\n\n" +
				"https://docs.Artee VPN.io/how-to/register-machines-using-setup-keys")
		}
	}
}
