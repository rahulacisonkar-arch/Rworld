...
func TestSSHCommand_FlagHandling(t *testing.T) {
	// Test that flags after hostname are not parsed by Artee VPN but passed to SSH command
	tests := []struct {
		name         string
		args         []string
		expectedHost string
		...
		{
			name:         "ls with -la flag should not be parsed by Artee VPN",
			args:         []string{"debian2", "ls", "-la"},
			expectedHost: "debian2",
			expectedCmd:  "ls -la",
			expectError:  false,
			description:  "ls -la should be passed as SSH command, not parsed as Artee VPN flags",
		},
		{
			name:         "command with Artee VPN-like flags should be passed through",
			args:         []string{"hostname", "echo", "--help"},
			expectedHost: "hostname",
			expectedCmd:  "echo --help",
			expectError:  false,
			description:  "--help should be passed to echo, not parsed by Artee VPN",
		},
		{
...
func TestSSHCommand_RegressionFlagParsing(t *testing.T) {
	// Regression test for the specific issue: "sudo ./Artee VPN ssh debian2 ls -la"
	// should not parse -la as Artee VPN flags but pass them to the SSH command
	tests := []struct {
		name         string
		args         []string
		expectedHost string
