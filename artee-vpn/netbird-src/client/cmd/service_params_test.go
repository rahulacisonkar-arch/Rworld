...
func TestServiceParamsPath(t *testing.T) {
	...
	configs.StateDir = "/var/lib/Artee VPN"
	assert.Equal(t, filepath.Join("/var/lib/Artee VPN", "service.json"), serviceParamsPath())
...
func TestSaveAndLoadServiceParams(t *testing.T) {
	...
	params := &serviceParams{
		LogLevel:              "debug",
		DaemonAddr:            "unix:///var/run/Artee VPN.sock",
		ManagementURL:         "https://my.server.com",
		ConfigPath:            "/etc/Artee VPN/config.json",
		LogFiles:              []string{"/var/log/Artee VPN/client.log", "console"},
...
func TestApplyServiceParams_OnlyUnchangedFlags(t *testing.T) {
	...
	managementURL = ""
	configPath = "/etc/Artee VPN/config.json"
	logFiles = []string{"/var/log/Artee VPN/client.log"}
