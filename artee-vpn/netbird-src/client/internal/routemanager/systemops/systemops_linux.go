...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/internal/routemanager/sysctl"
	"github.com/Artee VPNio/Artee VPN/client/internal/routemanager/vars"
...
const (
	// Artee VPNVPNTableID is the ID of the custom routing table used by Artee VPN.
	Artee VPNVPNTableID = 0x1BD0
	// Artee VPNVPNTableName is the name of the custom routing table used by Artee VPN.
	Artee VPNVPNTableName = "Artee VPN"
...
func getSetupRules() []ruleParams {
	return []ruleParams{
		...
		{110, nbnet.ControlPlaneMark, Artee VPNVPNTableID, netlink.FAMILY_V4, true, -1, "rule v4 Artee VPN"},
		{110, nbnet.ControlPlaneMark, Artee VPNVPNTableID, netlink.FAMILY_V6, true, -1, "rule v6 Artee VPN"},
	}
}
...
//
// Rule 2 (VPN Traffic Routing): Directs all remaining traffic to the 'Artee VPNVPNTableID' custom routing table.
...
func (r *SysOps) CleanupRouting(stateManager *statemanager.Manager, advancedRouting bool) error {
	...
	if err := flushRoutes(Artee VPNVPNTableID, netlink.FAMILY_V4); err != nil {
		result = multierror.Append(result, fmt.Errorf("flush routes v4: %w", err))
	}
	if err := flushRoutes(Artee VPNVPNTableID, netlink.FAMILY_V6); err != nil {
...
func (r *SysOps) AddVPNRoute(prefix netip.Prefix, intf *net.Interface) error {
	...
	if prefix == vars.Defaultv4 && (r.wgInterface == nil || !r.wgInterface.Address().HasIPv6()) {
		if err := addUnreachableRoute(vars.Defaultv6, Artee VPNVPNTableID); err != nil {
			return fmt.Errorf("add v6 blackhole: %w", err)
		}
	}
	if err := addRoute(prefix, Nexthop{netip.Addr{}, intf}, Artee VPNVPNTableID); err != nil {
...
func (r *SysOps) RemoveVPNRoute(prefix netip.Prefix, intf *net.Interface) error {
	...
	if prefix == vars.Defaultv4 && (r.wgInterface == nil || !r.wgInterface.Address().HasIPv6()) {
		if err := removeUnreachableRoute(vars.Defaultv6, Artee VPNVPNTableID); err != nil {
			log.Debugf("remove v6 blackhole: %v", err)
		}
	}
	if err := removeRoute(prefix, Nexthop{netip.Addr{}, intf}, Artee VPNVPNTableID); err != nil {
...
func getAllRoutingTables() ([]int, error) {
	...
	standardTables := []int{syscall.RT_TABLE_MAIN, syscall.RT_TABLE_LOCAL, Artee VPNVPNTableID}
...
// and verifies if existing names start with "Artee VPN_".
func entryExists(file *os.File, id int) (bool, error) {
	...
	for scanner.Scan() {
		...
		if _, err := fmt.Sscanf(line, "%d %s\n", &existingID, &existingName); err == nil {
			if existingID == id {
				if existingName != Artee VPNVPNTableName {
...
func addRoutingTableName() error {
	...
	exists, err := entryExists(file, Artee VPNVPNTableID)
	if err != nil {
		return fmt.Errorf("verify entry %d, %s: %w", Artee VPNVPNTableID, Artee VPNVPNTableName, err)
	}
	if exists {
		return nil
	}
	...
	if _, err := file.WriteString(fmt.Sprintf("\n%d\t%s\n", Artee VPNVPNTableID, Artee VPNVPNTableName)); err != nil {
