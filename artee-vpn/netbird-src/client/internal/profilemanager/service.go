...
func init() {
	...
	DefaultConfigPathDir = "/var/lib/Artee VPN/"
	oldDefaultConfigPathDir = "/etc/Artee VPN/"
	...
	if stateDir := os.Getenv("NB_STATE_DIR"); stateDir != "" {
		...
		switch runtime.GOOS {
		case "windows":
			oldDefaultConfigPathDir = filepath.Join(os.Getenv("PROGRAMDATA"), "Artee VPN")
			DefaultConfigPathDir = oldDefaultConfigPathDir

		case "freebsd":
			oldDefaultConfigPathDir = "/var/db/Artee VPN/"
			DefaultConfigPathDir = oldDefaultConfigPathDir
		}
	}
