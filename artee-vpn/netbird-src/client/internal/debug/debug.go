...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/anonymize"
	"github.com/Artee VPNio/Artee VPN/client/configs"
	...
	nbstatus "github.com/Artee VPNio/Artee VPN/client/status"
	mgmProto "github.com/Artee VPNio/Artee VPN/shared/management/proto"
	"github.com/Artee VPNio/Artee VPN/shared/netiputil"
)

const readmeContent = `Artee VPN debug bundle
...
status.txt: Anonymized status information of the Artee VPN client.
client.log: Most recent, anonymized client log file of the Artee VPN client.
Artee VPN.err: Most recent, anonymized stderr log file of the Artee VPN client.
Artee VPN.out: Most recent, anonymized stdout log file of the Artee VPN client.
...
sysctls.txt: Forwarding, reverse-path filter, source-validation, and conntrack accounting sysctl values that the Artee VPN client may read or modify, if --system-info flag was provided (Linux only).
resolv.conf: DNS resolver configuration from /etc/resolv.conf (Unix systems only), if --system-info flag was provided.
scutil_dns.txt: DNS configuration from scutil --dns (macOS only), if --system-info flag was provided.
resolved_domains.txt: Anonymized resolved domain IP addresses from the status recorder.
config.txt: Anonymized configuration information of the Artee VPN client.
network_map.json: Anonymized sync response containing peer configurations, routes, DNS settings, and firewall rules.
state.json: Anonymized client state dump containing Artee VPN states for the active profile.
...
Domains
All domain names (except for the Artee VPN domains) are replaced with randomly generated strings ending in ".domain". Anonymized domains are consistent across all files in the bundle.
...
State File
The state.json file contains anonymized internal state information of the Artee VPN client, including:
...
The goroutine, block, mutex, and heap profiling files contain process information that might help the Artee VPN team diagnose performance or memory issues. The information in these files doesn't contain personal data.
...
- Table: Routing table name (main, local, Artee VPN, etc.)
...
Resolved Domains
The resolved_domains.txt file contains information about domain names that have been resolved to IP addresses by Artee VPN's DNS resolver. This includes:
...
Configuration
The config.txt file contains anonymized configuration information of the Artee VPN client. Sensitive information such as private keys and SSH keys are excluded. The following fields are anonymized:
- ManagementURL
- AdminURL
- NATExternalIPs
- CustomDNSAddress
...
sysctls.txt:
- Forwarding (IPv4 + IPv6, global and per-interface), reverse-path filter, source-validation, conntrack accounting, and TCP-related sysctls that Artee VPN may read or modify
...
- Table: Target routing table name (main, local, Artee VPN, etc.)
...
const (
	...
	errorLogFile  = "Artee VPN.err"
	stdoutLogFile = "Artee VPN.out"

	darwinErrorLogPath  = "/var/log/Artee VPN.out.log"
	darwinStdoutLogPath = "/var/log/Artee VPN.err.log"
)
...
func (g *BundleGenerator) Generate() (resp string, err error) {
	bundlePath, err := os.CreateTemp(g.tempDir, "Artee VPN.debug.*.zip")
	if err != nil {
...
func (g *BundleGenerator) addConfig() error {
	...
	// GetConfig RPC consumed by `Artee VPN debug config`.
...
func (g *BundleGenerator) addCommonConfigFields(configContent *strings.Builder) {
	configContent.WriteString("Artee VPN Client Configuration:\n\n")
...
func (g *BundleGenerator) maskSecrets() {
	if g.syncResponse == nil || g.syncResponse.Artee VPNConfig == nil {
		return
	}

	if g.syncResponse.Artee VPNConfig.Flow != nil {
		g.syncResponse.Artee VPNConfig.Flow.TokenPayload = maskedValue

	}

	if g.syncResponse.Artee VPNConfig.Relay != nil {
		g.syncResponse.Artee VPNConfig.Relay.TokenPayload = maskedValue
	}

	for i := range g.syncResponse.Artee VPNConfig.Turns {
		if g.syncResponse.Artee VPNConfig.Turns[i] != nil {
			g.syncResponse.Artee VPNConfig.Turns[i].Password = maskedValue
		}
	}
}
...
func anonymizeArtee VPNConfig(config *mgmProto.Artee VPNConfig, anonymizer *anonymize.Anonymizer) {
...
func anonymizeSyncResponse(syncResponse *mgmProto.SyncResponse, anonymizer *anonymize.Anonymizer) error {
	if syncResponse.Artee VPNConfig != nil {
		anonymizeArtee VPNConfig(syncResponse.Artee VPNConfig, anonymizer)
	}
