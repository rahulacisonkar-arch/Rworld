...
func NewArtee VPNHandler() *Artee VPNHandler {
	return &Artee VPNHandler{
		peers:            map[rp.PeerID]wireGuardPeer{},
		initializedPeers: map[rp.PeerID]bool{},
	}
}
...
func (h *Artee VPNHandler) SetInterface(iface PresharedKeySetter) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.iface = iface
}

func (h *Artee VPNHandler) AddPeer(pid rp.PeerID, intf string, pk rp.Key) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *Artee VPNHandler) RemovePeer(pid rp.PeerID) {
	h.mu.Lock()
	defer h.mu.Unlock()
	delete(h.peers, pid)
...
func (h *Artee VPNHandler) IsPeerInitialized(pid rp.PeerID) bool {
	h.mu.Lock()
	defer h.mu.Unlock()
	return h.initializedPeers[pid]
}

func (h *Artee VPNHandler) HandshakeCompleted(pid rp.PeerID, key rp.Key) {
	h.outputKey(rp.KeyOutputReasonStale, pid, key)
}

func (h *Artee VPNHandler) HandshakeExpired(pid rp.PeerID) {
	key, _ := rp.GeneratePresharedKey()
	h.outputKey(rp.KeyOutputReasonStale, pid, key)
}

func (h *Artee VPNHandler) outputKey(_ rp.KeyOutputReason, pid rp.PeerID, psk rp.Key) {
	h.mu.Lock()
	iface := h.iface
	wg, ok := h.peers[pid]
