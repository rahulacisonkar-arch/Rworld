...
// ServerDomains represents the management server domains extracted from Artee VPN configuration
...
// ExtractFromArtee VPNConfig extracts domain information from Artee VPN protobuf configuration
func ExtractFromArtee VPNConfig(config *mgmProto.Artee VPNConfig) ServerDomains {
	if config == nil {
...
// extractSignalDomain extracts the signal domain from Artee VPN configuration.
func extractSignalDomain(config *mgmProto.Artee VPNConfig) domain.Domain {
...
// extractRelayDomains extracts relay server domains from Artee VPN configuration.
func extractRelayDomains(config *mgmProto.Artee VPNConfig) []domain.Domain {
	if config.Relay != nil {
...
// extractFlowDomain extracts the traffic flow domain from Artee VPN configuration.
func extractFlowDomain(config *mgmProto.Artee VPNConfig) domain.Domain {
	if config.Flow != nil {
...
// extractStunDomains extracts STUN server domains from Artee VPN configuration.
func extractStunDomains(config *mgmProto.Artee VPNConfig) []domain.Domain {
	var urls []string
...
// extractTurnDomains extracts TURN server domains from Artee VPN configuration.
func extractTurnDomains(config *mgmProto.Artee VPNConfig) []domain.Domain {
	var urls []string
