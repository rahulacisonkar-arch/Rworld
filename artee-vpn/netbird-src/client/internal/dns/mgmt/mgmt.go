...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/internal/dns/resutil"
...
// chain instead of net.DefaultResolver, avoiding loopback when Artee VPN is the
// system resolver.
...
// Resolver caches critical Artee VPN infrastructure domains.
...
func (m *Resolver) ServeDNS(w dns.ResponseWriter, r *dns.Msg) {
	...
	if inflight != nil && inflight.CompareAndSwap(false, true) {
		log.Warnf("mgmt cache: possible resolver loop for domain=%s: served stale while an OS-fallback refresh was inflight (if Artee VPN is the system resolver, the OS-path predicate is wrong)",
			question.Name)
	}
...
// for Artee VPN infrastructure domains (signal, relay, flow, etc.), not their subdomains.
...
func (m *Resolver) lookupBoth(ctx context.Context, d domain.Domain, dnsName string) (aRecords, aaaaRecords []dns.RR, errA, errAAAA error) {
	...
	// today: no root handler at priority ≤ PriorityUpstream means Artee VPN is
