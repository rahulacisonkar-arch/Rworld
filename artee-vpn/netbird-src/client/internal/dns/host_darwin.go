...
const (
	Artee VPNDNSStateKeyFormat            = "State:/Network/Service/Artee VPN-%s/DNS"
	Artee VPNDNSStateKeyIndexedFormat     = "State:/Network/Service/Artee VPN-%s-%d/DNS"
...
// discoverExistingKeys probes scutil for all Artee VPN DNS keys that may exist.
...
func (s *systemConfigurator) discoverExistingKeys() []string {
	...
	for _, suffix := range []string{searchSuffix, matchSuffix, localSuffix} {
		key := getKeyWithInput(Artee VPNDNSStateKeyFormat, suffix)
	...
	for _, suffix := range []string{searchSuffix, matchSuffix} {
		for i := 0; ; i++ {
			key := fmt.Sprintf(Artee VPNDNSStateKeyIndexedFormat, suffix, i)
			if !strings.Contains(dnsKeys, key) {
				break
			}
			keys = append(keys, key)
