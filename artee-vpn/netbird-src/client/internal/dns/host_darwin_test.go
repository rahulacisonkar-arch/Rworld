...
func TestDarwinDNSUncleanShutdownCleanup(t *testing.T) {
	...
	localKey := getKeyWithInput(Artee VPNDNSStateKeyFormat, localSuffix)
...
func TestMatchDomainBatching(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var got []string
			for i := range batches {
				key := fmt.Sprintf(Artee VPNDNSStateKeyIndexedFormat, matchSuffix, i)
				exists, err := checkDNSKeyExists(key)
				require.NoError(t, err)
...
func setupTestConfigurator(t *testing.T) (*systemConfigurator, *statemanager.Manager, func()) {
	...
	cleanup := func() {
		...
		_ = removeTestDNSKey(getKeyWithInput(Artee VPNDNSStateKeyFormat, searchSuffix))
		_ = removeTestDNSKey(getKeyWithInput(Artee VPNDNSStateKeyFormat, matchSuffix))
		_ = removeTestDNSKey(getKeyWithInput(Artee VPNDNSStateKeyFormat, localSuffix))
	}
...
func TestOriginalNameserversNoTransition(t *testing.T) {
	Artee VPNIP := netip.MustParseAddr("100.64.0.1")
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, srv := range initialServers {
				require.NotEqual(t, Artee VPNIP, srv, "initial servers should not contain Artee VPN IP")
			}

			config := HostDNSConfig{
				ServerIP:   Artee VPNIP,
				ServerPort: 53,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, srv := range servers {
				assert.NotEqual(t, Artee VPNIP, srv, "servers should not contain Artee VPN IP")
			}
		})
	}
}
