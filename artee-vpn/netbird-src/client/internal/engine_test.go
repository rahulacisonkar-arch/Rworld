...
import (
	...
	mgmt "github.com/Artee VPNio/Artee VPN/shared/management/client"
...
func startManagement(t *testing.T, dataDir, testFile string) (*grpc.Server, string, error) {
	...
	networkMapController := controller.NewController(context.Background(), store, metrics, updateManager, requestBuffer, server.MockIntegratedValidator{}, settingsMockManager, "Artee VPN.selfhosted", port_forwarding.NewControllerMock(), manager.NewEphemeralManager(store, peersManager), config)
