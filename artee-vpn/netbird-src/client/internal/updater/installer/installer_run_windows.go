...
const (
	daemonName    = "Artee VPN.exe"
	uiName        = "Artee VPN-ui.exe"
	updaterBinary = "updater.exe"

	msiLogFile = "msi.log"

	msiDownloadURL = "https://github.com/Artee VPNio/Artee VPN/releases/download/v%version/Artee VPN_installer_%version_windows_%arch.msi"
	exeDownloadURL = "https://github.com/Artee VPNio/Artee VPN/releases/download/v%version/Artee VPN_installer_%version_windows_%arch.exe"
)

var (
	defaultTempDir = filepath.Join(os.Getenv("ProgramData"), "Artee VPN", "tmp-install")
...
func (u *Installer) startDaemon(daemonFolder string) error {
	...
	if output, err := cmd.CombinedOutput(); err != nil {
		log.Debugf("failed to start Artee VPN service: %v, output: %s", err, string(output))
		return err
	}
...
func (u *Installer) startUIAsUser(daemonFolder string) error {
	...
	log.Infof("starting Artee VPN-ui: %s", uiPath)
	...
	log.Infof("Artee VPN-ui started successfully in session %d", sessionID)
	return nil
}
