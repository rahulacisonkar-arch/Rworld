...
BUCKET_ID=$(influx bucket list --org Artee VPN --name metrics --json | grep -oP '"id"\s*:\s*"\K[^"]+' | head -1)
ORG_ID=$(influx org list --name Artee VPN --json | grep -oP '"id"\s*:\s*"\K[^"]+' | head -1)
