...
Internal documentation for the Artee VPN client metrics system.
...
Measurement: `Artee VPN_peer_connection`
...
- `version`: Artee VPN version string
...
| `NB_METRICS_SERVER_URL` | *(from remote config)* | Ingest server URL (e.g., `https://ingest.Artee VPN.io`) |
...
| `NB_METRICS_CONFIG_URL` | `https://ingest.Artee VPN.io/config` | Remote push config URL |
...
| `INFLUXDB_URL` | `http://influxdb:8086/api/v2/write?org=Artee VPN&bucket=metrics&precision=ns` | InfluxDB write endpoint |
...
- **InfluxDB dashboard:** http://localhost:3001/d/Artee VPN-influxdb-metrics
