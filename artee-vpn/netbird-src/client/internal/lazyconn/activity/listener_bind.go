...
import (
	...
	"github.com/Artee VPNio/Artee VPN/client/internal/lazyconn"
)
...
// It automatically derives a unique fake IP from the peer's Artee VPN IP in the 127.2.x.x range.
...
// deriveFakeIP creates a deterministic fake IP for bind mode based on peer's Artee VPN IP.
// Maps peer IP 100.64.x.y to fake IP 127.2.x.y (similar to relay proxy using 127.1.x.y).
// It finds the peer's actual Artee VPN IP by checking which allowedIP is in the same subnet as our WG interface.
...
func deriveFakeIP(wgIface WgInterface, allowedIPs []netip.Prefix) (netip.Addr, error) {
	...
	return netip.Addr{}, fmt.Errorf("no peer Artee VPN IP found in allowed IPs")
}
