...
const (
	edeArtee VPNUpstreamTimeout uint16 = 49152
	edeArtee VPNUpstreamFailure uint16 = 49153
)
...
// edeCodeFor maps an upstream lookup error to the Artee VPN EDE info code.
func edeCodeFor(dnsErr *net.DNSError) uint16 {
	if dnsErr != nil && dnsErr.IsTimeout {
		return edeArtee VPNUpstreamTimeout
	}
	return edeArtee VPNUpstreamFailure
}
...
func edeText(dnsErr *net.DNSError) string {
	if dnsErr != nil && dnsErr.IsTimeout {
		return "Artee VPN forwarder: upstream timeout"
	}
	return "Artee VPN forwarder: upstream failure"
}
