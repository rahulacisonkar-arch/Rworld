...
func (e *Engine) setupSSHPortRedirection() error {
	...
	if !localAddr.IsValid() {
		return errors.New("invalid local Artee VPN address")
	}
...
// cleanupSSHConfig removes Artee VPN SSH client configuration on shutdown
...
func (e *Engine) startSSHServer(jwtConfig *sshserver.JWTConfig) error {
	...
	Artee VPNIP := wgAddr.IP
	listenAddr := netip.AddrPortFrom(Artee VPNIP, sshserver.InternalSSHPort)
