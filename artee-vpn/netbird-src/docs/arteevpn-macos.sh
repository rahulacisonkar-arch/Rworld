...
#
# SYNOPSIS
#   Push the Artee VPN MDM policy to a macOS device via JumpCloud Commands.
#
# DESCRIPTION
#   This is the macOS counterpart of docs/Artee VPN-policy.reg.ps1.
#   It writes the values declared in the "POLICY VALUES" block below to
#   the managed-preferences plist that the Artee VPN daemon's
#   client/mdm/policy_darwin.go loader reads on every 1-minute MDM
#   reload tick:
#
#       /Library/Managed Preferences/io.Artee VPN.client.plist
#
...
#   companion docs/Artee VPN-macos.mobileconfig as a Custom Configuration
...
#   docs/Artee VPN-macos.mobileconfig (sample profile)
#   docs/Artee VPN.admx + .adml (Windows ADMX schema)
#
NULL='__UNSET__'
managementURL='https://api.Artee VPN.io:443'
...
readonly PLIST_PATH="$PLIST_DIR/io.Artee VPN.client.plist"
readonly LOG_TAG='Artee VPN-mdm'
...
# main builds the Artee VPN MDM plist from configured policy variables, validates and installs it to /Library/Managed Preferences/io.Artee VPN.client.plist (root:wheel, 644) and optionally triggers the Artee VPN daemon to reload.
main() {
  log "applying Artee VPN MDM policy to $PLIST_PATH"
  ...
  log "policy installed; Artee VPN daemon will pick it up within the next 1-minute reload tick"

  # Optional: kick the daemon for an immediate apply. Safe — does
  # nothing on a host where Artee VPN is not yet installed.
  /bin/launchctl kickstart -k system/io.Artee VPN.client 2>/dev/null || true
}
