...
create_service_user() {
  ...
  RESPONSE=$(
    curl -sS -X POST "$INSTANCE_URL/management/v1/users/machine" \
      ...
      -H "Content-Type: application/json" \
      -d '{
            "userName": "Artee VPN-service-account",
            "name": "Artee VPN Service Account",
            "description": "Artee VPN Service Account for IDP management",
...
init_zitadel() {
  echo -e "\nInitializing Zitadel with Artee VPN's applications\n"
  INSTANCE_URL="$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN"
  ...
  BASE_REDIRECT_URL=$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN
  if [[ $Artee VPN_HTTP_PROTOCOL == "http" ]]; then
  ...
  ZITADEL_ADMIN_USERNAME="admin@$Artee VPN_DOMAIN"
  ...
  export Artee VPN_AUTH_CLIENT_ID=$DASHBOARD_APPLICATION_CLIENT_ID
  export Artee VPN_AUTH_CLIENT_ID_CLI=$CLI_APPLICATION_CLIENT_ID
  export Artee VPN_IDP_MGMT_CLIENT_ID=$SERVICE_USER_CLIENT_ID
  export Artee VPN_IDP_MGMT_CLIENT_SECRET=$SERVICE_USER_CLIENT_SECRET
...
initEnvironment() {
  ...
  Artee VPN_RELAY_AUTH_SECRET=$(openssl rand -base64 32 | sed 's/=//g')
  TURN_MIN_PORT=49152
  TURN_MAX_PORT=65535
  ...
  cat <<EOF
{
    ...
    "HttpConfig": {
        "AuthIssuer": "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN",
        "AuthAudience": "$Artee VPN_AUTH_CLIENT_ID",
        "OIDCConfigEndpoint":"$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/.well-known/openid-configuration"
    },
    "IdpManagerConfig": {
        "ManagerType": "zitadel",
        "ClientConfig": {
            "Issuer": "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN",
            "TokenEndpoint": "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/oauth/v2/token",
            "ClientID": "$Artee VPN_IDP_MGMT_CLIENT_ID",
            "ClientSecret": "$Artee VPN_IDP_MGMT_CLIENT_SECRET",
            "GrantType": "client_credentials"
        },
        "ExtraConfig": {
            "ManagementEndpoint": "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/management/v1"
        }
    },
  "DeviceAuthorizationFlow": {
      "Provider": "hosted",
      "ProviderConfig": {
          "Audience": "$Artee VPN_AUTH_CLIENT_ID_CLI",
          "ClientID": "$Artee VPN_AUTH_CLIENT_ID_CLI",
          "Scope": "openid"
      }
    },
...
# OIDC
AUTH_AUDIENCE=$Artee VPN_AUTH_CLIENT_ID
AUTH_CLIENT_ID=$Artee VPN_AUTH_CLIENT_ID
AUTH_AUTHORITY=$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN
USE_AUTH0=false
...
ZITADEL_EXTERNALPORT=$Artee VPN_PORT
ZITADEL_EXTERNALDOMAIN=$Artee VPN_DOMAIN
...
services:
  ...
  coturn:
    image: coturn/coturn
    restart: unless-stopped
    #domainname: Artee VPN.relay.selfhosted
    volumes:
  ...
  zitadel:
    ...
    volumes:
      ...
      - Artee VPN_zitadel_certs:/zdb-certs:ro
    logging:
      driver: "json-file"
      options:
        max-size: "500m"
...
renderDockerComposeCockroachDB() {
  ...
  zdb:
    ...
    volumes:
      - Artee VPN_zdb_data:/cockroach/cockroach-data
      - Artee VPN_zdb_certs:/cockroach/certs
      - Artee VPN_zitadel_certs:/zitadel-certs
    healthcheck:
...
renderDockerComposePostgres() {
  ...
  zdb:
    ...
    env_file:
      - ./zdb.env
    volumes:
      - Artee VPN_zdb_data:/var/lib/postgresql/data:rw
    healthcheck:
