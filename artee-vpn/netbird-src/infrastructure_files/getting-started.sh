...
# Artee VPN Getting Started with Embedded IdP (Dex)
# This script sets up Artee VPN with the embedded Dex identity provider
...
check_nb_domain() {
  DOMAIN=$1
  if [[ "$DOMAIN-x" == "-x" ]]; then
    echo "The Artee VPN_DOMAIN variable cannot be empty." > /dev/stderr
    return 1
  fi

  if [[ "$DOMAIN" == "Artee VPN.example.com" ]]; then
    echo "The Artee VPN_DOMAIN cannot be Artee VPN.example.com" > /dev/stderr
    return 1
  fi
  return 0
}
...
read_nb_domain() {
  READ_Artee VPN_DOMAIN=""
  echo -n "Enter the domain you want to use for Artee VPN (e.g. Artee VPN.my-domain.com): " > /dev/stderr
  read -r READ_Artee VPN_DOMAIN < /dev/tty
  if ! check_nb_domain "$READ_Artee VPN_DOMAIN"; then
    read_nb_domain
  fi
  echo "$READ_Artee VPN_DOMAIN"
  return 0
}
...
read_traefik_network() {
  ...
  echo -n "External network (leave empty to create 'Artee VPN' network): " > /dev/stderr
...
read_proxy_docker_network() {
  ...
  echo "If yes, enter the Docker network ${proxy_name} is on (Artee VPN will join it)." > /dev/stderr
...
read_enable_proxy() {
  echo "" > /dev/stderr
  echo "Do you want to enable the Artee VPN Proxy service?" > /dev/stderr
  echo "The proxy allows you to selectively expose internal Artee VPN network resources" > /dev/stderr
...
wait_management_proxy() {
  ...
  echo -n "Waiting for Artee VPN server to become ready"
  counter=1
  while true; do
    # Check the embedded IdP endpoint through the reverse proxy
    if curl -sk -f -o /dev/null "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/oauth2/.well-known/openid-configuration" 2>/dev/null; then
      break
    fi
    if [[ $counter -eq 60 ]]; then
      ...
      $DOCKER_COMPOSE_COMMAND logs --tail=20 Artee VPN-server
    fi
    echo -n " ."
    sleep 2
    counter=$((counter + 1))
...
initialize_default_values() {
  Artee VPN_PORT=80
  Artee VPN_HTTP_PROTOCOL="http"
  Artee VPN_RELAY_PROTO="rel"
  Artee VPN_RELAY_AUTH_SECRET=$(openssl rand -base64 32 | sed "$SED_STRIP_PADDING")
  ...
  Artee VPN_STUN_PORT=3478

  # Docker images
  DASHBOARD_IMAGE=${DASHBOARD_IMAGE:-"Artee VPNio/dashboard:latest"}
  # Combined server replaces separate signal, relay, and management containers
  Artee VPN_SERVER_IMAGE=${Artee VPN_SERVER_IMAGE:-"Artee VPNio/Artee VPN-server:latest"}
  Artee VPN_PROXY_IMAGE=${Artee VPN_PROXY_IMAGE:-"Artee VPNio/reverse-proxy:latest"}
...
configure_domain() {
  if ! check_nb_domain "$Artee VPN_DOMAIN"; then
    Artee VPN_DOMAIN=$(read_nb_domain)
  fi

  if [[ "$Artee VPN_DOMAIN" == "use-ip" ]]; then
    Artee VPN_DOMAIN=$(get_main_ip_address)
    BASE_DOMAIN=$Artee VPN_DOMAIN
  else
    Artee VPN_PORT=443
    Artee VPN_HTTP_PROTOCOL="https"
    Artee VPN_RELAY_PROTO="rels"
    BASE_DOMAIN=$(echo $Artee VPN_DOMAIN | sed -E 's/^[^.]+\.//')
  fi
  return 0
}
...
check_existing_installation() {
  if [[ -f config.yaml ]]; then
    ...
    echo "  rm -f docker-compose.yml dashboard.env config.yaml proxy.env traefik-dynamic.yaml nginx-Artee VPN.conf caddyfile-Artee VPN.txt npm-advanced-config.txt && rm -rf crowdsec/"
  ...
  case "$REVERSE_PROXY_TYPE" in
    0)
      ...
      if [[ "$ENABLE_PROXY" == "true" ]]; then
        ...
        # This will be overwritten with the actual token after Artee VPN-server starts
        echo "# Placeholder - will be updated with token after Artee VPN-server starts" > proxy.env
    ...
    2)
      ...
      render_nginx_conf > nginx-Artee VPN.conf
      ;;
    3)
      render_docker_compose_exposed_ports > docker-compose.yml
    ...
    4)
      render_docker_compose_exposed_ports > docker-compose.yml
      render_external_caddyfile > caddyfile-Artee VPN.txt
      ;;
    5)
      render_docker_compose_exposed_ports > docker-compose.yml
      ;;
...
start_services_and_show_instructions() {
  ...
  if [[ "$REVERSE_PROXY_TYPE" == "0" ]]; then
    ...
    if [[ "$ENABLE_PROXY" == "true" ]]; then
      ...
      local core_services="traefik dashboard Artee VPN-server"
      if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
      ...
      PROXY_TOKEN=$($DOCKER_COMPOSE_COMMAND exec -T Artee VPN-server \
        /go/bin/Artee VPN-server token create --name "default-proxy" --config /etc/Artee VPN/config.yaml 2>/dev/null | grep "^Token:" | awk '{print $2}')

      if [[ -z "$PROXY_TOKEN" ]]; then
        echo "ERROR: Failed to create proxy token. Check Artee VPN-server logs." > /dev/stderr
        $DOCKER_COMPOSE_COMMAND logs --tail=20 Artee VPN-server
        exit 1
      fi
      ...
      if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
        ...
        while ! $DOCKER_COMPOSE_COMMAND exec -T crowdsec cscli lapi status >/dev/null 2>&1; do
          ...
          if [[ $cs_retries -ge 30 ]]; then
            ...
            echo "  docker exec Artee VPN-crowdsec cscli bouncers add Artee VPN-proxy -o raw" > /dev/stderr
            ENABLE_CROWDSEC="false"
            break
          fi
          sleep 2
        ...
        if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
          CROWDSEC_BOUNCER_KEY=$($DOCKER_COMPOSE_COMMAND exec -T crowdsec \
            cscli bouncers add Artee VPN-proxy -o raw 2>/dev/null)
    ...
    print_post_setup_instructions
    echo ""
    echo "Artee VPN containers are running. Once Traefik is connected, access the dashboard at:"
    echo "  $Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN"
    ...
    print_post_setup_instructions
    echo ""
    echo "Artee VPN containers are running. Configure NPM as shown above, then access:"
    echo "  $Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN"
  else
    ...
    echo -e "$MSG_DONE"
    echo "Artee VPN is now running. Access the dashboard at:"
    echo "  $Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN"
  fi
  return 0
}
...
render_docker_compose_traefik_builtin() {
  ...
  if [[ "$ENABLE_PROXY" == "true" ]]; then
    ...
    proxy_service="
  # Artee VPN Proxy - exposes internal resources to the internet
  proxy:
    image: $Artee VPN_PROXY_IMAGE
    container_name: Artee VPN-proxy
    ports:
    - 51820:51820/udp
...
"
    proxy_volumes="
  Artee VPN_proxy_certs:"

    if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
      crowdsec_service="
  crowdsec:
    image: $CROWDSEC_IMAGE
    container_name: Artee VPN-crowdsec
    restart: unless-stopped
    networks: [Artee VPN]
    environment:
...
services:
  ...
  traefik:
    image: $TRAEFIK_IMAGE
    container_name: Artee VPN-traefik
    restart: unless-stopped
    networks:
      Artee VPN:
    ...
    command:
      ...
      - "--providers.docker.network=Artee VPN"
      # Entrypoints
...
$traefik_file_provider
    ...
    volumes:
      ...
      - Artee VPN_traefik_letsencrypt:/letsencrypt
$traefik_dynamic_volume
  ...
  dashboard:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.Artee VPN-dashboard.rule=Host(\`$Artee VPN_DOMAIN\`)
      - traefik.http.routers.Artee VPN-dashboard.entrypoints=websecure
      - traefik.http.routers.Artee VPN-dashboard.tls=true
      - traefik.http.routers.Artee VPN-dashboard.tls.certresolver=letsencrypt
      - traefik.http.routers.Artee VPN-dashboard.service=dashboard
      - traefik.http.routers.Artee VPN-dashboard.priority=1
  ...
  Artee VPN-server:
    image: $Artee VPN_SERVER_IMAGE
    container_name: Artee VPN-server
    restart: unless-stopped
    networks: [Artee VPN]
    ports:
      - '$Artee VPN_STUN_PORT:$Artee VPN_STUN_PORT/udp'
    volumes:
    ...
    labels:
      ...
      - traefik.http.routers.Artee VPN-grpc.rule=Host(\`$Artee VPN_DOMAIN\`) && (PathPrefix(\`/signalexchange.SignalExchange/\`) || PathPrefix(\`/management.ManagementService/\`) || PathPrefix(\`/management.ProxyService/\`))
      - traefik.http.routers.Artee VPN-grpc.entrypoints=websecure
      - traefik.http.routers.Artee VPN-grpc.tls=true
      - traefik.http.routers.Artee VPN-grpc.tls.certresolver=letsencrypt
      - traefik.http.routers.Artee VPN-grpc.service=Artee VPN-server-h2c
      - traefik.http.routers.Artee VPN-grpc.priority=100
      # Backend router (relay, WebSocket, API, OAuth2)
      - traefik.http.routers.Artee VPN-backend.rule=Host(\`$Artee VPN_DOMAIN\`) && (PathPrefix(\`/relay\`) || PathPrefix(\`/ws-proxy/\`) || PathPrefix(\`/api\`) || PathPrefix(\`/oauth2\`))
      - traefik.http.routers.Artee VPN-backend.entrypoints=websecure
      - traefik.http.routers.Artee VPN-backend.tls=true
      - traefik.http.routers.Artee VPN-backend.tls.certresolver=letsencrypt
      - traefik.http.routers.Artee VPN-backend.service=Artee VPN-server
      - traefik.http.routers.Artee VPN-backend.priority=100
      # Services
      - traefik.http.services.Artee VPN-server.loadbalancer.server.port=80
      - traefik.http.services.Artee VPN-server-h2c.loadbalancer.server.port=80
      - traefik.http.services.Artee VPN-server-h2c.loadbalancer.server.scheme=h2c
    logging:
      driver: "json-file"
      options:
        max-size: "500m"
...
volumes:
  Artee VPN_data:
  Artee VPN_traefik_letsencrypt:${proxy_volumes}${crowdsec_volumes}
...
render_combined_yaml() {
  cat <<EOF
# Combined Artee VPN Server Configuration (Simplified)
# Generated by getting-started.sh

server:
  listenAddress: ":80"
  exposedAddress: "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN:$Artee VPN_PORT"
  stunPorts:
    - $Artee VPN_STUN_PORT
  metricsPort: 9090
  ...
  auth:
    issuer: "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/oauth2"
    signKeyRefreshEnabled: true
    dashboardRedirectURIs:
      - "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/nb-auth"
      - "$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/nb-silent-auth"
    cliRedirectURIs:
...
render_dashboard_env() {
  cat <<EOF
# Endpoints
Artee VPN_MGMT_API_ENDPOINT=$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN
Artee VPN_MGMT_GRPC_API_ENDPOINT=$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN
# OIDC - using embedded IdP
AUTH_AUDIENCE=Artee VPN-dashboard
AUTH_CLIENT_ID=Artee VPN-dashboard
AUTH_CLIENT_SECRET=
AUTH_AUTHORITY=$Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN/oauth2
USE_AUTH0=false
...
NB_PROXY_MANAGEMENT_ADDRESS=http://Artee VPN-server:80
...
NB_PROXY_DOMAIN=$Artee VPN_DOMAIN
...
render_docker_compose_traefik() {
  local network_name="${TRAEFIK_EXTERNAL_NETWORK:-Artee VPN}"
...
services:
  ...
  dashboard:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.Artee VPN-dashboard.rule=Host(\`$Artee VPN_DOMAIN\`)
      - traefik.http.routers.Artee VPN-dashboard.entrypoints=$TRAEFIK_ENTRYPOINT
      - traefik.http.routers.Artee VPN-dashboard.tls=true
$(if [[ -n "$tls_labels" ]]; then echo "      - traefik.http.routers.Artee VPN-dashboard.${tls_labels}"; fi)
      - traefik.http.routers.Artee VPN-dashboard.priority=1
      - traefik.http.services.Artee VPN-dashboard.loadbalancer.server.port=80
    logging:
      driver: "json-file"
      options:
        max-size: "500m"
  ...
  Artee VPN-server:
    ...
    labels:
      - traefik.enable=true
      # gRPC router (needs h2c backend for HTTP/2 cleartext)
      - traefik.http.routers.Artee VPN-grpc.rule=Host(\`$Artee VPN_DOMAIN\`) && (PathPrefix(\`/signalexchange.SignalExchange/\`) || PathPrefix(\`/management.ManagementService/\`))
      - traefik.http.routers.Artee VPN-grpc.entrypoints=$TRAEFIK_ENTRYPOINT
      - traefik.http.routers.Artee VPN-grpc.tls=true
$(if [[ -n "$tls_labels" ]]; then echo "      - traefik.http.routers.Artee VPN-grpc.${tls_labels}"; fi)
      - traefik.http.routers.Artee VPN-grpc.service=Artee VPN-server-h2c
      # Backend router (relay, WebSocket, API, OAuth2)
      - traefik.http.routers.Artee VPN-backend.rule=Host(\`$Artee VPN_DOMAIN\`) && (PathPrefix(\`/relay\`) || PathPrefix(\`/ws-proxy/\`) || PathPrefix(\`/api\`) || PathPrefix(\`/oauth2\`))
      - traefik.http.routers.Artee VPN-backend.entrypoints=$TRAEFIK_ENTRYPOINT
      - traefik.http.routers.Artee VPN-backend.tls=true
$(if [[ -n "$tls_labels" ]]; then echo "      - traefik.http.routers.Artee VPN-backend.${tls_labels}"; fi)
      - traefik.http.routers.Artee VPN-backend.service=Artee VPN-server
      # Services
      - traefik.http.services.Artee VPN-server.loadbalancer.server.port=80
      - traefik.http.services.Artee VPN-server-h2c.loadbalancer.server.port=80
...
render_docker_compose_exposed_ports() {
  ...
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    networks="[Artee VPN, $EXTERNAL_PROXY_NETWORK]"
...
#    Debian/Ubuntu: /etc/nginx/sites-available/Artee VPN (then symlink to sites-enabled)
#    RHEL/CentOS:   /etc/nginx/conf.d/Artee VPN.conf
# 3. Test and reload: nginx -t && systemctl reload nginx"

  # If running in Docker network, use container names
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    dashboard_addr="Artee VPN-dashboard:80"
    server_addr="Artee VPN-server:80"
...
# Add this to your nginx.conf or include it from a separate file."
  ...
  cat <<EOF
# Artee VPN Nginx Configuration
# Generated by getting-started.sh
#
${install_note}

upstream Artee VPN_dashboard {
    server ${dashboard_addr};
    keepalive 10;
}
upstream Artee VPN_server {
    server ${server_addr};
}

server {
    listen 80;
    server_name $Artee VPN_DOMAIN;
...
server {
    ...
    #   ssl_certificate /etc/letsencrypt/live/$Artee VPN_DOMAIN/fullchain.pem;
    #   ssl_certificate_key /etc/letsencrypt/live/$Artee VPN_DOMAIN/privkey.pem;
    #
    # Let's Encrypt (acme.sh):
    #   ssl_certificate /root/.acme.sh/$Artee VPN_DOMAIN/fullchain.cer;
    #   ssl_certificate_key /root/.acme.sh/$Artee VPN_DOMAIN/$Artee VPN_DOMAIN.key;
    #
    # Custom certificates:
    #   ssl_certificate /etc/ssl/certs/$Artee VPN_DOMAIN.crt;
    #   ssl_certificate_key /etc/ssl/private/$Artee VPN_DOMAIN.key;
    #
    ...
    location ~ ^/(relay|ws-proxy/) {
        proxy_pass http://Artee VPN_server;
    ...
    location ~ ^/(signalexchange\.SignalExchange|management\.ManagementService)/ {
        grpc_pass grpc://Artee VPN_server;
    ...
    # Dashboard (catch-all)
    location / {
        proxy_pass http://Artee VPN_dashboard;
    }
}
EOF
  return 0
...
print_builtin_traefik_instructions() {
  echo ""
  echo "$MSG_SEPARATOR"
  echo "  Artee VPN SETUP COMPLETE"
  echo "$MSG_SEPARATOR"
  echo ""
  echo "You can access the Artee VPN dashboard at:"
  echo "  $Artee VPN_HTTP_PROTOCOL://$Artee VPN_DOMAIN"
  echo ""
  echo "Follow the onboarding steps to set up your Artee VPN instance."
  echo ""
  ...
  echo ""
  echo "Open ports:"
  echo "  - 443/tcp   (HTTPS - all Artee VPN services)"
  echo "  - 80/tcp    (HTTP - redirects to HTTPS)"
  echo "  - $Artee VPN_STUN_PORT/udp   (STUN - required for NAT traversal)"
  if [[ "$ENABLE_PROXY" == "true" ]]; then
  ...
  echo ""
  echo "  Commercial license: https://Artee VPN.io/pricing#on-prem"
  echo "  Scaling guide:      https://docs.Artee VPN.io/scaling-your-self-hosted-deployment"
  echo ""
  if [[ "$ENABLE_PROXY" == "true" ]]; then
    echo "Artee VPN Proxy:"
    echo "  The proxy service is enabled and running."
    echo "  Any domain NOT matching $Artee VPN_DOMAIN will be passed through to the proxy."
    ...
    echo ""
    echo "  *.$Artee VPN_DOMAIN    CNAME    $Artee VPN_DOMAIN"
    echo ""
    if [[ "$ENABLE_CROWDSEC" == "true" ]]; then
      ...
      echo "    docker exec Artee VPN-crowdsec cscli console enroll <your-enrollment-key>"
...
print_traefik_instructions() {
  ...
  echo "$MSG_SEPARATOR"
  echo ""
  echo "Artee VPN containers are configured with Traefik labels."
  echo ""
  echo "Configuration:"
...
print_nginx_instructions() {
  ...
  echo "$MSG_SEPARATOR"
  echo ""
  echo "Generated: nginx-Artee VPN.conf"
  echo ""
  ...
  echo ""
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    echo "Artee VPN containers have joined the '$EXTERNAL_PROXY_NETWORK' Docker network."
    ...
    echo "  2. Edit nginx-Artee VPN.conf and update SSL certificate paths"
    ...
    echo "  2. Edit nginx-Artee VPN.conf and update certificate paths"
    ...
    echo "https://docs.Artee VPN.io/selfhosted/reverse-proxy#tls-certificate-setup-for-nginx"
    echo ""
    echo "Container ports (bound to ${bind_addr}):"
    echo "  Dashboard:     ${DASHBOARD_HOST_PORT}"
    echo "  Artee VPN Server: ${MANAGEMENT_HOST_PORT} (all services)"
  fi
  return 0
}
...
print_npm_instructions() {
  ...
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    ...
    echo "  Forward Hostname: Artee VPN-dashboard"
...
print_external_caddy_instructions() {
  ...
  echo "$MSG_SEPARATOR"
  echo ""
  echo "Generated: caddyfile-Artee VPN.txt"
  echo ""
  if [[ -n "$EXTERNAL_PROXY_NETWORK" ]]; then
    echo "Artee VPN containers have joined the '$EXTERNAL_PROXY_NETWORK' Docker network."
    echo "The config uses container names for upstream servers."
    echo ""
    echo "$MSG_NEXT_STEPS"
    echo "  1. Add the contents of caddyfile-Artee VPN.txt to your Caddyfile"
    echo "  2. Reload Caddy"
  else
    echo "$MSG_NEXT_STEPS"
    echo "  1. Add the contents of caddyfile-Artee VPN.txt to your Caddyfile"
...
print_manual_instructions() {
  ...
  echo "Container ports (bound to ${bind_addr}):"
  echo "  Dashboard:     ${DASHBOARD_HOST_PORT}"
  echo "  Artee VPN Server: ${MANAGEMENT_HOST_PORT} (all services: management, signal, relay)"
  echo ""
