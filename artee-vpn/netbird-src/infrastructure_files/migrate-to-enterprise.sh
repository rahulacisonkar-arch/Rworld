...
# Artee VPN — community combined → Enterprise combined migration
#
...
detect_combined_service() {
  yq eval '.services | to_entries | map(select(.value.image | test("^Artee VPNio/Artee VPN-server"))) | .[0].key // ""' "$COMPOSE_FILE"
}

detect_dashboard_service() {
  yq eval '.services | to_entries | map(select(.value.image | test("^Artee VPNio/dashboard"))) | .[0].key // ""' "$COMPOSE_FILE"
}

detect_config_yaml_host_path() {
  yq eval ".services[\"$COMBINED_SERVICE\"].volumes[] | select(. | test(\":/etc/Artee VPN/config.yaml\")) | sub(\":/etc/Artee VPN/config.yaml.*\"; \"\") // \"\"" "$COMPOSE_FILE" | head -1
}

detect_data_volume() {
  yq eval ".services[\"$COMBINED_SERVICE\"].volumes[] | select(. | test(\":/var/lib/Artee VPN\")) | sub(\":/var/lib/Artee VPN.*\"; \"\") // \"\"" "$COMPOSE_FILE" | head -1
}
...
services:
  ${DASHBOARD_SERVICE}:
    image: \${Artee VPN_DASHBOARD_IMAGE:-ghcr.io/Artee VPNio/dashboard-cloud:latest}

  ${COMBINED_SERVICE}:
    image: \${Artee VPN_SERVER_IMAGE:-ghcr.io/Artee VPNio/Artee VPN-server-cloud:latest}
    environment:
...
EOF
  ...
  if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
    ...
    volumes:
      - ./${ENTERPRISE_CONFIG_FILE}:/etc/Artee VPN/config.yaml.enterprise:ro
    command: ["--config", "/etc/Artee VPN/config.yaml.enterprise"]
  ...
  postgres:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U Artee VPN -d Artee VPN"]
      interval: 5s
      timeout: 5s
      retries: 20
EOF
  ...
  flow-enricher:
    image: ghcr.io/Artee VPNio/flow-enricher-cloud:latest
    container_name: Artee VPN-flow-enricher
    restart: unless-stopped
    networks: [${COMPOSE_NETWORK}]
    depends_on:
      postgres:
    ...
    environment:
      ...
      NB_MANAGEMENT_POSTGRES_DSN: "host=postgres user=Artee VPN password=\${POSTGRES_PASSWORD} dbname=Artee VPN port=5432 sslmode=disable"
      NB_STORE_ENGINE_POSTGRES_DSN: "host=postgres user=Artee VPN password=\${POSTGRES_PASSWORD} dbname=Artee VPN port=5432 sslmode=disable"
      NB_TRAFFIC_EVENT_STORE_ENGINE: postgres
      NB_TRAFFIC_EVENT_POSTGRES_DSN: "host=postgres user=Artee VPN password=\${POSTGRES_PASSWORD} dbname=Artee VPN port=5432 sslmode=disable"
  ...
  flow-receiver:
    image: ghcr.io/Artee VPNio/flow-receiver-cloud:latest
    container_name: Artee VPN-flow-receiver
    restart: unless-stopped
    networks: [${COMPOSE_NETWORK}]
    depends_on:
      nats:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.Artee VPN-flow.rule=Host(\`${Artee VPN_HOSTNAME}\`) && PathPrefix(\`/flow.FlowService/\`)
      - traefik.http.routers.Artee VPN-flow.entrypoints=websecure
      - traefik.http.routers.Artee VPN-flow.tls=true
      - traefik.http.routers.Artee VPN-flow.tls.certresolver=letsencrypt
      - traefik.http.routers.Artee VPN-flow.service=Artee VPN-flow-h2c
      - traefik.http.routers.Artee VPN-flow.priority=100
      - traefik.http.services.Artee VPN-flow-h2c.loadbalancer.server.port=80
      - traefik.http.services.Artee VPN-flow-h2c.loadbalancer.server.scheme=h2c
EOF
  fi
...
EOF
    if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
      echo "  Artee VPN_postgres:"
    fi
    if [[ "$ENABLE_FLOW" == "yes" ]]; then
      echo "  Artee VPN_nats_data:"
    fi
  fi
}
...
render_enterprise_config() {
  local pg_dsn="host=postgres user=Artee VPN password=${POSTGRES_PASSWORD} dbname=Artee VPN port=5432 sslmode=disable"
  ...
  if [[ "$ENABLE_FLOW" == "yes" ]]; then
    local flow_addr="${Artee VPN_DOMAIN}"
    yq eval -i "
...
backup_sqlite() {
  ...
  docker run --rm \
    -v "${data_volume_actual}:/var/lib/Artee VPN:ro" \
    -v "${BACKUP_DIR}:/backup" \
    busybox \
    sh -c 'cp -a /var/lib/Artee VPN/. /backup/ 2>/dev/null || true'
  local copied
...
run_migrate_store() {
  ...
  $DOCKER_COMPOSE_COMMAND run --rm "$COMBINED_SERVICE" migrate-store --config /etc/Artee VPN/config.yaml.enterprise --verify
  echo "  done"
}
...
init_migration() {
  ...
  if [[ -z "$COMBINED_SERVICE" ]]; then
    echo "Could not find a service running Artee VPNio/Artee VPN-server* in $COMPOSE_FILE." > /dev/stderr
    echo "This script targets the community combined-server deployment." > /dev/stderr
    exit 1
  fi
  if [[ -z "$DASHBOARD_SERVICE" ]]; then
    echo "Could not find a service running Artee VPNio/dashboard* in $COMPOSE_FILE." > /dev/stderr
    exit 1
  fi
  if [[ -z "$CONFIG_YAML_HOST" ]]; then
    echo "Could not find a config.yaml mount on $COMBINED_SERVICE (expected to bind-mount to /etc/Artee VPN/config.yaml)." > /dev/stderr
    exit 1
  fi
  ...
  if [[ -z "$DATA_VOLUME" ]]; then
    echo "Could not find a volume mounted at /var/lib/Artee VPN on $COMBINED_SERVICE." > /dev/stderr
    exit 1
  fi
  ...
  if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
    ...
    if [[ "$ENABLE_FLOW" == "yes" ]]; then
      ...
      Artee VPN_DOMAIN=$(detect_exposed_address)
      if [[ -z "$Artee VPN_DOMAIN" ]] || [[ "$Artee VPN_DOMAIN" == "null" ]]; then
        Artee VPN_DOMAIN=$(read_required "  Public Artee VPN URL (e.g. https://Artee VPN.example.com)")
      fi
      # Strip protocol + port to leave just the hostname for the Traefik Host() rule.
      Artee VPN_HOSTNAME=$(echo "$Artee VPN_DOMAIN" | sed -E 's,^https?://,,' | sed 's,:.*,,' | sed 's,/.*,,')

      # We need the encryption key from the existing config.yaml for the enricher
      Artee VPN_ENCRYPTION_KEY=$(yq eval '.server.store.encryptionKey // ""' "$CONFIG_YAML_HOST")
      if [[ -z "$Artee VPN_ENCRYPTION_KEY" ]] || [[ "$Artee VPN_ENCRYPTION_KEY" == "null" ]]; then
...
apply_changes() {
  ...
  if [[ -z "${Artee VPN_LICENSE_SERVER_BASE_URL:-}" ]]; then
    sed -i.bak '/Artee VPN_LICENSE_SERVER_BASE_URL/d' "$OVERRIDE_FILE" && rm -f "$OVERRIDE_FILE.bak"
  fi
  ...
  chmod 600 "$ENV_FILE"
  {
    ...
    if [[ -n "${Artee VPN_LICENSE_SERVER_BASE_URL:-}" ]]; then
      echo "Artee VPN_LICENSE_SERVER_BASE_URL=${Artee VPN_LICENSE_SERVER_BASE_URL}"
    fi
    if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
    ...
    if [[ "$ENABLE_FLOW" == "yes" ]]; then
      ...
      echo "Artee VPN_ENCRYPTION_KEY=${Artee VPN_ENCRYPTION_KEY}"
    fi
  } >> "$ENV_FILE"
  ...
  if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
    ...
    while ! $DOCKER_COMPOSE_COMMAND exec -T postgres pg_isready -U Artee VPN -d Artee VPN &> /dev/null; do
      echo -n " ."
      sleep 2
      counter=$((counter + 1))
...
print_summary() {
  ...
  if [[ "$MIGRATE_POSTGRES" == "yes" ]]; then
    ...
    pg_volume=$(resolve_data_volume "Artee VPN_postgres")
    data_volume_actual=$(resolve_data_volume "$DATA_VOLUME")
    ...
    echo "  docker run --rm -v ${data_volume_actual}:/var/lib/Artee VPN -v ${BACKUP_DIR}:/backup busybox sh -c 'cp -a /backup/. /var/lib/Artee VPN/'"
  fi
