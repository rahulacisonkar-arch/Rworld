#!/bin/bash
#
# Artee VPN Migration Script: Pre-v0.65.0 → Combined Container Setup
#
# Migrates from the old 5-container deployment (dashboard, signal, relay, management, coturn)
# to the new 2-container setup (Traefik + combined Artee VPN-server).
#
...
#
# Usage:
#   ./migrate.sh [--install-dir /path/to/Artee VPN] [--non-interactive]
...
readonly DASHBOARD_IMAGE="Artee VPNio/dashboard:latest"
readonly Artee VPN_SERVER_IMAGE="Artee VPNio/Artee VPN-server:latest"
...
print_banner() {
  echo ""
  echo "$MSG_SEPARATOR"
  echo "  Artee VPN Migration Tool v${SCRIPT_VERSION}"
...
detect_install_dir() {
  ...
  local search_paths=("$PWD" "/opt/Artee VPN" "/opt/wiretrustee")
  ...
  echo ""
  echo -n "Enter the path to your Artee VPN installation directory: "
...
validate_old_setup() {
  ...
  if [[ -f "$INSTALL_DIR/management.json" ]]; then
    ...
    echo "This doesn't appear to be a valid Artee VPN installation."
    exit 1
  fi
...
detect_reverse_proxy() {
  ...
  if [[ -f "$INSTALL_DIR/setup.env" ]] && grep -q 'Artee VPN_DISABLE_LETSENCRYPT=true' "$INSTALL_DIR/setup.env" 2>/dev/null; then
    PROXY_TYPE="external"
...
detect_idp_type() {
  ...
  if [[ -n "$manager_type" && "$manager_type" != "null" && "$manager_type" != "none" && "$manager_type" != "" ]]; then
    ...
    echo ""
    echo "Please refer to the Artee VPN documentation for upgrade instructions:"
    echo "  https://docs.Artee VPN.io/selfhosted/getting-started"
    exit 1
  fi
...
detect_volumes() {
  ...
  # Check compose-prefixed patterns (e.g., Artee VPN_Artee VPN-mgmt, infrastructure_files_Artee VPN-mgmt)
  local compose_prefixed
  compose_prefixed=$(echo "$volumes_list" | grep -E '(Artee VPN|wiretrustee).*mgmt' | head -n1 || echo "")
  ...
  if [[ -n "$compose_file" ]]; then
    # Look for volume mount on /var/lib/Artee VPN in management or Artee VPN-server service
    local vol_name
    vol_name=$(grep -E '^\s+-\s+\S+:/var/lib/Artee VPN' "$compose_file" 2>/dev/null | head -1 | sed 's/.*- //' | sed 's/:.*//' | tr -d ' ' || echo "")
...
detect_domain() {
  ...
  if [[ -z "$DOMAIN" && -f "$INSTALL_DIR/setup.env" ]]; then
    DOMAIN=$(grep '^Artee VPN_DOMAIN=' "$INSTALL_DIR/setup.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || echo "")
  fi
  ...
  # Try dashboard.env Artee VPN_MGMT_API_ENDPOINT
  if [[ -z "$DOMAIN" && -f "$INSTALL_DIR/dashboard.env" ]]; then
    local endpoint
    endpoint=$(grep '^Artee VPN_MGMT_API_ENDPOINT=' "$INSTALL_DIR/dashboard.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || echo "")
    if [[ -n "$endpoint" ]]; then
  ...
  if [[ -f "$INSTALL_DIR/setup.env" ]]; then
    LETSENCRYPT_EMAIL=$(grep '^Artee VPN_LETSENCRYPT_EMAIL=' "$INSTALL_DIR/setup.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || echo "")
  fi
...
detect_store_config() {
  ...
  if [[ -f "$INSTALL_DIR/setup.env" ]]; then
    local pg_dsn
    pg_dsn=$(grep '^Artee VPN_STORE_ENGINE_POSTGRES_DSN=' "$INSTALL_DIR/setup.env" 2>/dev/null | sed 's/^Artee VPN_STORE_ENGINE_POSTGRES_DSN=//' | tr -d '"' || echo "")
    if [[ -n "$pg_dsn" ]]; then
      STORE_DSN="$pg_dsn"
    fi

    local mysql_dsn
    mysql_dsn=$(grep '^Artee VPN_STORE_ENGINE_MYSQL_DSN=' "$INSTALL_DIR/setup.env" 2>/dev/null | sed 's/^Artee VPN_STORE_ENGINE_MYSQL_DSN=//' | tr -d '"' || echo "")
    if [[ -n "$mysql_dsn" ]]; then
      STORE_DSN="$mysql_dsn"
    fi
  fi
  ...
  if [[ -z "$STORE_DSN" && -f "$INSTALL_DIR/base.setup.env" ]]; then
    local pg_dsn
    pg_dsn=$(grep '^Artee VPN_STORE_ENGINE_POSTGRES_DSN=' "$INSTALL_DIR/base.setup.env" 2>/dev/null | sed 's/^Artee VPN_STORE_ENGINE_POSTGRES_DSN=//' | tr -d '"' || echo "")
    if [[ -n "$pg_dsn" ]]; then
      STORE_DSN="$pg_dsn"
    fi

    local mysql_dsn
    mysql_dsn=$(grep '^Artee VPN_STORE_ENGINE_MYSQL_DSN=' "$INSTALL_DIR/base.setup.env" 2>/dev/null | sed 's/^Artee VPN_STORE_ENGINE_MYSQL_DSN=//' | tr -d '"' || echo "")
    if [[ -n "$mysql_dsn" ]]; then
      STORE_DSN="$mysql_dsn"
    fi
  fi
...
extract_config_values() {
  ...
  if [[ (-z "$RELAY_SECRET" || "$RELAY_SECRET" == "null") && -f "$INSTALL_DIR/setup.env" ]]; then
    RELAY_SECRET=$(grep '^Artee VPN_RELAY_AUTH_SECRET=' "$INSTALL_DIR/setup.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || echo "")
  fi

  # Fallback: relay secret from base.setup.env
  if [[ (-z "$RELAY_SECRET" || "$RELAY_SECRET" == "null") && -f "$INSTALL_DIR/base.setup.env" ]]; then
    RELAY_SECRET=$(grep '^Artee VPN_RELAY_AUTH_SECRET=' "$INSTALL_DIR/base.setup.env" 2>/dev/null | cut -d'=' -f2 | tr -d '"' | tr -d "'" || echo "")
  fi
...
create_backup() {
  ...
  # Record state
  {
    echo "# Artee VPN migration backup state"
    echo "# Created: $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
    echo ""
    echo "## Docker volumes"
    docker volume ls --format '{{.Name}}' 2>/dev/null | grep -E '(Artee VPN|wiretrustee)' || echo "(none found)"
    echo ""
    echo "## Running containers"
    docker ps --format '{{.Names}}\t{{.Image}}\t{{.Status}}' 2>/dev/null | grep -E '(Artee VPN|wiretrustee|dashboard|signal|relay|management|coturn)' || echo "(none running)"
...
# Artee VPN Migration Rollback Script
...
echo "Restoring Artee VPN configuration from backup..."
...
# Endpoints
Artee VPN_MGMT_API_ENDPOINT=https://${DOMAIN}
Artee VPN_MGMT_GRPC_API_ENDPOINT=https://${DOMAIN}
...
services:
  ...
  dashboard:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.Artee VPN-dashboard.rule=Host(\`${DOMAIN}\`)
  ...
  Artee VPN-server:
    ...
    labels:
      ...
      - traefik.http.routers.Artee VPN-grpc.rule=Host(\`${DOMAIN}\`) && (PathPrefix(\`/signalexchange.SignalExchange/\`) || PathPrefix(\`/management.ManagementService/\`))
      ...
      - traefik.http.routers.Artee VPN-backend.rule=Host(\`${DOMAIN}\`) && (PathPrefix(\`/relay\`) || PathPrefix(\`/ws-proxy/\`) || PathPrefix(\`/api\`) || PathPrefix(\`/oauth2\`))
...
wait_for_health() {
  ...
  while [[ $attempt -lt $max_attempts ]]; do
    ...
    if [[ $attempt -eq 30 ]]; then
      ...
      (cd "$INSTALL_DIR" && $DOCKER_COMPOSE_CMD logs --tail=10 Artee VPN-server 2>/dev/null) || true
...
print_summary() {
  ...
  echo "    - Clients should reconnect automatically; if not: Artee VPN down && Artee VPN up"
  echo ""
  echo "  Next steps:"
  echo "    - Access the dashboard: https://$DOMAIN"
  echo "    - Re-authenticate all clients: Artee VPN down && Artee VPN up"
  ...
  while [[ $# -gt 0 ]]; do
    ...
    case "$arg" in
      ...
      --non-interactive)
        ...
        ;;
      --help|-h)
        echo "Usage: $0 [--install-dir /path/to/Artee VPN] [--non-interactive]"
        echo ""
        echo "Migrates a pre-v0.65.0 Artee VPN deployment to the combined container setup."
        echo ""
        echo "Options:"
        echo "  --install-dir DIR    Path to existing Artee VPN installation"
