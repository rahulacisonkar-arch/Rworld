...
const (
	...
	PolicyRuleProtocolArtee VPNSsh PolicyRuleProtocol = "Artee VPN-ssh"
...
func (e PolicyRuleProtocol) Valid() bool {
	...
	case PolicyRuleProtocolIcmp:
		return true
	case PolicyRuleProtocolArtee VPNSsh:
		return true
...
const (
	...
	PolicyRuleMinimumProtocolArtee VPNSsh PolicyRuleMinimumProtocol = "Artee VPN-ssh"
...
func (e PolicyRuleMinimumProtocol) Valid() bool {
	...
	case PolicyRuleMinimumProtocolIcmp:
		return true
	case PolicyRuleMinimumProtocolArtee VPNSsh:
		return true
...
const (
	...
	PolicyRuleUpdateProtocolArtee VPNSsh PolicyRuleUpdateProtocol = "Artee VPN-ssh"
...
func (e PolicyRuleUpdateProtocol) Valid() bool {
	...
	case PolicyRuleUpdateProtocolIcmp:
		return true
	case PolicyRuleUpdateProtocolArtee VPNSsh:
		return true
...
type AccessiblePeer struct {
	...
	// DnsLabel Peer's DNS label is the parsed peer name for domain resolution. It is used to form an FQDN by appending the account's domain to the peer label. e.g. peer-dns-label.Artee VPN.cloud
	...
	// LastSeen Last time peer connected to Artee VPN's management service
...
// InstanceVersionInfo Version information for Artee VPN components
...
type ProxyCluster struct {
	...
	// Private True when at least one connected proxy in this cluster is running embedded in a Artee VPN client (`Artee VPN proxy`) and serving over a WireGuard tunnel. Lets the dashboard distinguish per-peer / private clusters from centralised ones.
	...
	// `shared` clusters are operated by Artee VPN and shared across accounts.
...
type ReverseProxyDomain struct {
	...
	// SupportsPrivate Whether the proxy cluster supports private (Artee VPN-only) services. True when at least one connected proxy in the cluster runs embedded in a Artee VPN client.
...
type Service struct {
	// AccessGroups Artee VPN group IDs whose peers may reach this private service over the tunnel. Required when private=true; ignored otherwise. Mutually exclusive with bearer auth (SSO).
	...
	// Private When true, the service is Artee VPN-only — its target points at a proxy cluster, inbound peers authenticate via their WireGuard tunnel identity (no OIDC), and an ACL policy is auto-generated from access_groups to the cluster's proxy-peer group. Requires mode=http.
	Private *bool `json:"private,omitempty"`
...
type ServiceTargetOptions struct {
	...
	// instead of through its embedded Artee VPN client. Use for upstreams
...
type User struct {
	...
	// Role User's Artee VPN account role
	Role string `json:"role"`
