...
import (
	...
	"github.com/Artee VPNio/Artee VPN/shared/management/client/rest"
...
func TestReverseProxyClusters_Delete_200(t *testing.T) {
	withMockClient(func(c *rest.Client, mux *http.ServeMux) {
		// PathEscape on "proxy.Artee VPN.local" leaves it intact; the route mux
		// matches the unescaped form. Sanity-check both the method and that
		// path-escaping doesn't double-encode the dotted address.
		mux.HandleFunc("/api/reverse-proxies/clusters/proxy.Artee VPN.local", func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(t, "DELETE", r.Method, "Delete must use DELETE")
			w.WriteHeader(200)
		})
		err := c.ReverseProxyClusters.Delete(context.Background(), "proxy.Artee VPN.local")
		require.NoError(t, err)
	})
}
