...
func startManagement(t *testing.T) (*grpc.Server, net.Listener) {
	...
	networkMapController := controller.NewController(ctx, store, metrics, updateManager, requestBuffer, mgmt.MockIntegratedValidator{}, settingsMockManager, "Artee VPN.selfhosted", port_forwarding.NewControllerMock(), ephemeral_manager.NewEphemeralManager(store, peersManger), config)
...
func TestClient_Sync(t *testing.T) {
	...
	case resp := <-ch:
		...
		if resp.GetArtee VPNConfig() == nil {
			t.Error("expecting non nil Artee VPNConfig got nil")
		}
...
func Test_SystemMetaDataFromClient(t *testing.T) {
	...
	expectedMeta := &mgmtProto.PeerSystemMeta{
		...
		Artee VPNVersion: info.Artee VPNVersion,
...
func isEqual(a, b *mgmtProto.PeerSystemMeta) bool {
	...
	return a.GetHostname() == b.GetHostname() &&
		...
		a.GetArtee VPNVersion() == b.GetArtee VPNVersion() &&
