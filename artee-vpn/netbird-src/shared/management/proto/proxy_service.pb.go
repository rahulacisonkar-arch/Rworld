...
type ProxyCapabilities struct {
	...
	// Whether the proxy is running embedded in the Artee VPN client and serving
	// exclusively over the WireGuard tunnel (i.e. `Artee VPN proxy` rather than
	// the standalone Artee VPN-proxy binary). Surfaces upstream so dashboards can
...
type PathTargetOptions struct {
	...
	// instead of through the embedded Artee VPN client. Useful for upstreams
...
type ProxyMapping struct {
	...
	// Artee VPN-only: the proxy MUST call ValidateTunnelPeer and fail closed; operator auth schemes are bypassed.
...
type ValidateSessionResponse struct {
	...
	// Stamped onto upstream requests as X-Artee VPN-Groups so downstream
