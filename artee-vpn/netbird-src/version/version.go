...
// DevelopmentVersion is the value of Artee VPNVersion() for non-release builds.
...
// Artee VPNVersion returns the Artee VPN version. For non-release builds the
// value is the literal DevelopmentVersion constant; the VCS revision is
// exposed separately via Artee VPNCommit so the wire format stays stable.
func Artee VPNVersion() string {
	return version
}

// Artee VPNCommit returns the VCS revision (truncated to 12 chars) of the
