...
func (h *handler) savePolicy(w http.ResponseWriter, r *http.Request, accountID string, userID string, policyID string, create bool) {
	...
	for _, rule := range req.Rules {
		...
		case api.PolicyRuleUpdateProtocolArtee VPNSsh:
			pr.Protocol = types.PolicyRuleProtocolArtee VPNSSH
		default:
		...
		if pr.Protocol == types.PolicyRuleProtocolArtee VPNSSH && rule.AuthorizedGroups != nil && len(*rule.AuthorizedGroups) != 0 {
			for _, sourceGroupID := range pr.Sources {
				_, ok := (*rule.AuthorizedGroups)[sourceGroupID]
				if !ok {
					util.WriteError(r.Context(), status.Errorf(status.InvalidArgument, "authorized group for Artee VPN-ssh protocol should be specified for each source group"), w)
					return
				}
			}
