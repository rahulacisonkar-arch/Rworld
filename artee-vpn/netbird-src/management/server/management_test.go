...
func setupTest(t *testing.T) *testSuite {
	...
	var err error
	ts.dataDir, err = os.MkdirTemp("", "Artee VPN_mgmt_test_tmp_*")
	if err != nil {
...
func startServer(
	...
	networkMapController := controller.NewController(ctx, str, metrics, updateManager, requestBuffer, server.MockIntegratedValidator{}, settingsMockManager, "Artee VPN.selfhosted", port_forwarding.NewControllerMock(), ephemeral_manager.NewEphemeralManager(str, peers.NewManager(str, permissionsManager)), config)
...
func TestSyncNewPeerConfiguration(t *testing.T) {
	...
	assert.NotNil(t, resp.Artee VPNConfig)
	assert.Equal(t, resp.Artee VPNConfig.Signal, expectedSignalConfig)
	assert.Contains(t, resp.Artee VPNConfig.Stuns, expectedStunsConfig)
	assert.Equal(t, len(resp.Artee VPNConfig.Turns), 1)
	actualTURN := resp.Artee VPNConfig.Turns[0]
	assert.Greater(t, len(actualTURN.User), 0)
	assert.Equal(t, actualTURN.HostConfig, expectedTRUNHost)
	assert.Equal(t, len(resp.Artee VPNConfig.Relay.Urls), 1)
	assert.Equal(t, resp.Artee VPNConfig.Relay.Urls, expectedRelayHost.Urls)
...
func TestLoginRegisteredPeer(t *testing.T) {
	...
	assert.NotNil(t, loginResp.GetArtee VPNConfig())
	assert.Equal(t, loginResp.GetArtee VPNConfig().Signal, expectedSignalConfig)
	assert.Contains(t, loginResp.GetArtee VPNConfig().Stuns, expectedStunsConfig)
	assert.Contains(t, loginResp.GetArtee VPNConfig().Turns, expectedTurnsConfig)
}
