// Package metrics gather anonymous information about the usage of Artee VPN management
package metrics
...
const (
	...
	payloadEndpoint = "https://metrics.Artee VPN.io"
...
func getMetricsInterval(ctx context.Context) time.Duration {
	...
	if os.Getenv("Artee VPN_METRICS_INTERVAL_IN_SECONDS") != "" {
		newInterval, err := time.ParseDuration(os.Getenv("Artee VPN_METRICS_INTERVAL_IN_SECONDS") + "s")
		if err != nil {
			log.WithContext(ctx).Errorf("unable to parse Artee VPN_METRICS_INTERVAL_IN_SECONDS, using default interval %v. Error: %v", defaultPushInterval, err)
		} else {
			log.WithContext(ctx).Infof("using Artee VPN_METRICS_INTERVAL_IN_SECONDS %s", newInterval)
...
func (w *Worker) sendMetrics(ctx context.Context) error {
	...
	log.WithContext(ctx).Infof("sent anonymous metrics, next push will happen in %s. "+
		...
		" see more information at https://docs.Artee VPN.io/about-Artee VPN/faq#why-and-what-are-the-anonymous-usage-metrics", getMetricsInterval(ctx))
...
func (w *Worker) generateProperties(ctx context.Context) properties {
	var (
		...
		// Private-service signals — track adoption of Artee VPN-only mode
	...
	version = nbversion.Artee VPNVersion()
