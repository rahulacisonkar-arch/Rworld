...
import (
	...
	"github.com/Artee VPNio/Artee VPN/encryption"
...
func Test_SyncProtocol(t *testing.T) {
	...
	mgmtServer, _, mgmtAddr, cleanup, err := startManagementForTest(t, "testdata/store_with_expired_peers.sql", &config.Config{
		Stuns: []*config.Host{{
			Proto: "udp",
			URI:   "stun:stun.Artee VPN.io:3468",
		}},
		TURNConfig: &config.TURNConfig{
			...
			Turns: []*config.Host{{
				Proto: "udp",
				URI:   "turn:stun.Artee VPN.io:3468",
			}},
		},
		Signal: &config.Host{
			Proto: "http",
			URI:   "signal.Artee VPN.io:10000",
		},
		Datadir:    dir,
		HttpConfig: nil,
	})
	...
	Artee VPNConfig := syncResp.GetArtee VPNConfig()
	if Artee VPNConfig == nil {
		t.Fatal("expecting SyncResponse to have non-nil Artee VPNConfig")
	}

	if Artee VPNConfig.GetSignal() == nil {
		t.Fatal("expecting SyncResponse to have Artee VPNConfig with non-nil Signal config")
	}
	...
	if Artee VPNConfig.GetSignal().GetUri() != expectedSignalConfig.GetUri() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected Signal URI: %v, actual: %v",
			expectedSignalConfig.GetUri(),
			Artee VPNConfig.GetSignal().GetUri())
	}

	if Artee VPNConfig.GetSignal().GetProtocol() != expectedSignalConfig.GetProtocol() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected Signal Protocol: %v, actual: %v",
			expectedSignalConfig.GetProtocol().String(),
			Artee VPNConfig.GetSignal().GetProtocol())
	}
	...
	if Artee VPNConfig.GetStuns()[0].GetUri() != expectedStunsConfig.GetUri() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected STUN URI: %v, actual: %v",
			expectedStunsConfig.GetUri(),
			Artee VPNConfig.GetStuns()[0].GetUri())
	}

	if Artee VPNConfig.GetStuns()[0].GetProtocol() != expectedStunsConfig.GetProtocol() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected STUN Protocol: %v, actual: %v",
			expectedStunsConfig.GetProtocol(),
			Artee VPNConfig.GetStuns()[0].GetProtocol())
	}
	...
	if Artee VPNConfig.GetTurns()[0].GetHostConfig().GetUri() != expectedTRUNHost.GetUri() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected TURN URI: %v, actual: %v",
			expectedTRUNHost.GetUri(),
			Artee VPNConfig.GetTurns()[0].GetHostConfig().GetUri())
	}

	if Artee VPNConfig.GetTurns()[0].GetHostConfig().GetProtocol() != expectedTRUNHost.GetProtocol() {
		t.Fatalf("expecting SyncResponse to have Artee VPNConfig with expected TURN Protocol: %v, actual: %v",
			expectedTRUNHost.GetProtocol().String(),
			Artee VPNConfig.GetTurns()[0].GetHostConfig().GetProtocol())
	}
...
func startManagementForTest(t *testing.T, testFile string, config *config.Config) (*grpc.Server, *DefaultAccountManager, string, func(), error) {
	...
	networkMapController := controller.NewController(ctx, store, metrics, updateManager, requestBuffer, MockIntegratedValidator{}, settingsMockManager, "Artee VPN.selfhosted", port_forwarding.NewControllerMock(), ephemeralMgr, config)
...
func Test_SyncStatusRace(t *testing.T) {
	...
	if os.Getenv("CI") == "true" {
		if os.Getenv("Artee VPN_STORE_ENGINE") == "postgres" {
			t.Skip("Skipping on CI and Postgres store")
		}

		if os.Getenv("Artee VPN_STORE_ENGINE") == "mysql" {
...
func Test_LoginPerformance(t *testing.T) {
	...
	t.Setenv("Artee VPN_STORE_ENGINE", "sqlite")
					...
					for i := 0; i < bc.peers; i++ {
						...
						peerLogin := types.PeerLogin{
							...
							Meta: nbpeer.PeerSystemMeta{
								...
								WtVersion:          meta.GetArtee VPNVersion(),
