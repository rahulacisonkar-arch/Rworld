...
import (
	...
	"github.com/Artee VPNio/Artee VPN/management/internals/modules/reverseproxy/domain"
	"github.com/Artee VPNio/Artee VPN/shared/management/status"

	nbdns "github.com/Artee VPNio/Artee VPN/dns"
	"github.com/Artee VPNio/Artee VPN/idp/dex"
	...
	"github.com/Artee VPNio/Artee VPN/management/internals/modules/reverseproxy/proxy"
	proxymanager "github.com/Artee VPNio/Artee VPN/management/internals/modules/reverseproxy/proxy/manager"
	...
	nbAccount "github.com/Artee VPNio/Artee VPN/management/server/account"
	"github.com/Artee VPNio/Artee VPN/management/server/activity"
	"github.com/Artee VPNio/Artee VPN/management/server/cache"
	"github.com/Artee VPNio/Artee VPN/management/server/http/testing/testing_tools"
	"github.com/Artee VPNio/Artee VPN/management/server/idp"
	...
	"github.com/Artee VPNio/Artee VPN/management/server/testutil"
...
func verifyCanAddPeerToAccount(t *testing.T, manager nbAccount.Manager, account *types.Account, userID string) {
	...
	peer := &nbpeer.Peer{
		...
		Name: "test-host@Artee VPN.io",
		Meta: nbpeer.PeerSystemMeta{
			Hostname:  "test-host@Artee VPN.io",
			GoOS:      "linux",
			Kernel:    "Linux",
			Core:      "21.04",
			Platform:  "x86_64",
...
func TestAccount_GetPeerNetworkMap(t *testing.T) {
	...
	network := &types.Network{
		...
		Dns:        "Artee VPN.selfhosted",
		Serial:     0,
		Mu:         sync.Mutex{},
	}

	for _, testCase := range tt {
		account := newAccountWithId(context.Background(), "account-1", userID, "Artee VPN.io", "", "", false)
		...
		customZone := account.GetPeersCustomZone(context.Background(), "Artee VPN.io")
...
func TestAccountManager_AddPeer(t *testing.T) {
	...
	userID := "testingUser"
	account, err := createAccount(manager, "test_account", userID, "Artee VPN.cloud")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccountManager_AddPeerWithUserID(t *testing.T) {
	...
	account, err := manager.GetOrCreateAccountByUser(context.Background(), auth.UserAuth{UserId: userID, Domain: "Artee VPN.cloud"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAccount_SetJWTGroups(t *testing.T) {
	t.Setenv("Artee VPN_STORE_ENGINE", "postgres")
	manager, _, err := createManager(t)
	require.NoError(t, err, "unable to create account manager")
...
func createManager(t testing.TB) (*DefaultAccountManager, *update_channel.PeersUpdateManager, error) {
	...
	networkMapController := controller.NewController(ctx, store, metrics, updateManager, requestBuffer, MockIntegratedValidator{}, settingsMockManager, "Artee VPN.cloud", port_forwarding.NewControllerMock(), ephemeral_manager.NewEphemeralManager(store, peers.NewManager(store, permissionsManager)), &config.Config{})
...
func TestMain(m *testing.M) {
	...
	if exitCode == 0 && os.Getenv("CI") == "true" {
		...
		storeEngine := os.Getenv("Artee VPN_STORE_ENGINE")
