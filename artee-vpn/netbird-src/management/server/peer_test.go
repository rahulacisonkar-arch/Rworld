...
import (
	...
	"github.com/Artee VPNio/Artee VPN/management/internals/controllers/network_map/controller/cache"
	...
	ephemeral_manager "github.com/Artee VPNio/Artee VPN/management/internals/modules/peers/ephemeral/manager"
	"github.com/Artee VPNio/Artee VPN/management/internals/server/config"
	"github.com/Artee VPNio/Artee VPN/management/internals/shared/grpc"
	nbcache "github.com/Artee VPNio/Artee VPN/management/server/cache"
	"github.com/Artee VPNio/Artee VPN/management/server/http/testing/testing_tools"
	...
	"github.com/Artee VPNio/Artee VPN/shared/auth"
	"github.com/Artee VPNio/Artee VPN/shared/management/status"

	"github.com/Artee VPNio/Artee VPN/management/server/util"

	resourceTypes "github.com/Artee VPNio/Artee VPN/management/server/networks/resources/types"
	routerTypes "github.com/Artee VPNio/Artee VPN/management/server/networks/routers/types"
	networkTypes "github.com/Artee VPNio/Artee VPN/management/server/networks/types"
	...
	nbroute "github.com/Artee VPNio/Artee VPN/route"
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, channel := range peerChannels {
				update := <-channel
				assert.Nil(t, update.Update.Artee VPNConfig)
...
func TestToSyncResponse(t *testing.T) {
	...
	checks := []*posture.Checks{
		{
			Checks: posture.ChecksDefinition{
				ProcessCheck: &posture.ProcessCheck{
					Processes: []posture.Process{{LinuxPath: "/usr/bin/Artee VPN"}},
				},
			},
		},
	}
	...
	// assert Artee VPN config
	assert.Equal(t, "signal.uri", response.Artee VPNConfig.Signal.Uri)
	assert.Equal(t, proto.HostConfig_HTTPS, response.Artee VPNConfig.Signal.GetProtocol())
	assert.Equal(t, "stun.uri", response.Artee VPNConfig.Stuns[0].Uri)
	assert.Equal(t, "turn.uri", response.Artee VPNConfig.Turns[0].HostConfig.GetUri())
	assert.Equal(t, "turn-user", response.Artee VPNConfig.Turns[0].User)
	assert.Equal(t, "turn-pass", response.Artee VPNConfig.Turns[0].Password)
	// assert RemotePeers
	...
	assert.Equal(t, "/usr/bin/Artee VPN", response.Checks[0].Files[0])
...
func Test_RegisterPeerByUser(t *testing.T) {
	...
	networkMapController := controller.NewController(ctx, s, metrics, updateManager, requestBuffer, MockIntegratedValidator{}, settingsMockManager, "Artee VPN.cloud", port_forwarding.NewControllerMock(), ephemeral_manager.NewEphemeralManager(s, peers.NewManager(s, permissionsManager)), &config.Config{})
...
func Test_RegisterPeerRollbackOnFailure(t *testing.T) {
	engine := os.Getenv("Artee VPN_STORE_ENGINE")
...
func Test_IsUniqueConstraintError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("Artee VPN_STORE_ENGINE", string(tt.engine))
			s, cleanup, err := store.NewTestStoreFromSQL(context.Background(), "testdata/extended-store.sql", t.TempDir())
			if err != nil {
...
func TestUpdatePeer_DnsLabelCollisionWithFQDN(t *testing.T) {
	...
	// Add first peer with hostname that produces DNS label "Artee VPN1"
	...
	peer1, _, _, _, err := manager.AddPeer(context.Background(), "", "", userID, &nbpeer.Peer{
		...
		Meta: nbpeer.PeerSystemMeta{Hostname: "Artee VPN1.Artee VPN.cloud"},
	}, false)
	require.NoError(t, err, "unable to add first peer")
	assert.Equal(t, "Artee VPN1", peer1.DNSLabel)
	...
	update := peer2.Copy()
	update.Name = "Artee VPN1.demo.Artee VPN.cloud"
	...
	assert.Equal(t, "Artee VPN1.demo.Artee VPN.cloud", updated.Name)
	assert.NotEqual(t, "Artee VPN1", updated.DNSLabel, "DNS label should not collide with existing peer")
	assert.Contains(t, updated.DNSLabel, "Artee VPN1-", "DNS label should be IP-based fallback")
}
