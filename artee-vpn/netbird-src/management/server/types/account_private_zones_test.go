...
import (
	...
	proxydomain "github.com/Artee VPNio/Artee VPN/management/internals/modules/reverseproxy/domain"
...
func TestSynthesizePrivateServiceZones_PeerInGroup_GetsRecord(t *testing.T) {
	...
	zone := zones[0]
	assert.Equal(t, "eu.proxy.Artee VPN.io.", zone.Domain, "zone apex must be the cluster FQDN")
	...
	rec := zone.Records[0]
	assert.Equal(t, "myapp.eu.proxy.Artee VPN.io.", rec.Name, "record name is the service FQDN")
...
func TestSynthesizePrivateServiceZones_PartiallyDisconnectedProxyPeers_OnlyConnectedSurface(t *testing.T) {
	...
	account.Peers["proxy-peer-2"] = &nbpeer.Peer{
		...
		ProxyMeta: nbpeer.ProxyMeta{Embedded: true, Cluster: "eu.proxy.Artee VPN.io"},
	}
...
func TestPrivateZone_GetPeerNetworkMapFromComponents_ShipsSynthZone(t *testing.T) {
	...
	zone, ok := findCustomZone(nm.DNSConfig.CustomZones, "eu.proxy.Artee VPN.io")
...
func TestPrivateZone_GetPeerNetworkMap_PeerOutsideGroups_OmitsSynthZone(t *testing.T) {
	...
	_, ok := findCustomZone(nm.DNSConfig.CustomZones, "eu.proxy.Artee VPN.io")
...
func TestSynthesizePrivateServiceZones_CustomDomain_ZoneApexIsRegisteredDomain(t *testing.T) {
	...
	account.Domains = []*proxydomain.Domain{
		{Domain: "example.com", AccountID: "acct-1", TargetCluster: "eu.proxy.Artee VPN.io", Validated: true},
	}
...
func TestSynthesizePrivateServiceZones_CustomAndFreeDomain_SeparateZones(t *testing.T) {
	...
	free, ok := findCustomZone(zones, "eu.proxy.Artee VPN.io")
	...
	assert.Equal(t, "myapp.eu.proxy.Artee VPN.io.", free.Records[0].Name, "cluster zone record is the free-domain FQDN")
...
func TestSynthesizePrivateServiceZones_CustomDomainClusterMismatch_NoZone(t *testing.T) {
	...
	account.Services[0].Domain = "app.example.com"
	account.Domains = []*proxydomain.Domain{
		{Domain: "example.com", AccountID: "acct-1", TargetCluster: "us.proxy.Artee VPN.io", Validated: true},
	}
...
func TestSynthesizePrivateServiceZones_TwoServicesSameCluster_OneZone(t *testing.T) {
	...
	account.Services = append(account.Services, &service.Service{
		...
		AccountID:    "acct-1",
		Name:         "anotherapp",
		Domain:       "anotherapp.eu.proxy.Artee VPN.io",
		ProxyCluster: "eu.proxy.Artee VPN.io",
		Enabled:      true,
		Private:      true,
		Mode:         service.ModeHTTP,
	...
	names := []string{zones[0].Records[0].Name, zones[0].Records[1].Name}
	assert.ElementsMatch(t, []string{"myapp.eu.proxy.Artee VPN.io.", "anotherapp.eu.proxy.Artee VPN.io."}, names, "both service domains must surface")
}
...
func TestSynthesizePrivateServiceZones_MixedClusterCustomAndPublic(t *testing.T) {
	...
	account.Services = []*service.Service{
		...
		privateService("cluster-1", "cluster1.eu.proxy.Artee VPN.io"),
		privateService("cluster-2", "cluster2.eu.proxy.Artee VPN.io"),
		privateService("cluster-3", "cluster3.eu.proxy.Artee VPN.io"),
		...
		publicService("public-cluster", "public.eu.proxy.Artee VPN.io"),
	...
	cluster, ok := findCustomZone(zones, "eu.proxy.Artee VPN.io")
	require.True(t, ok, "cluster-suffix services collapse into the cluster-apex zone")
	clusterNames := recordNames(cluster)
	assert.ElementsMatch(t,
		[]string{"cluster1.eu.proxy.Artee VPN.io.", "cluster2.eu.proxy.Artee VPN.io.", "cluster3.eu.proxy.Artee VPN.io."},
		clusterNames,
