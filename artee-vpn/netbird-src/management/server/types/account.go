...
func (a *Account) GetPeerConnectionResources(ctx context.Context, peer *nbpeer.Peer, validatedPeersMap map[string]struct{}, groupIDToUserIDs map[string][]string) ([]*nbpeer.Peer, []*FirewallRule, map[string]map[string]struct{}, bool) {
	...
	for _, policy := range a.Policies {
		...
		for _, rule := range policy.Rules {
			...
			if peerInDestinations && rule.Protocol == PolicyRuleProtocolArtee VPNSSH {
				sshEnabled = true
				switch {
...
func (a *Account) connResourcesGenerator(ctx context.Context, targetPeer *nbpeer.Peer) (func(*PolicyRule, []*nbpeer.Peer, int), func() ([]*nbpeer.Peer, []*FirewallRule)) {
	...
	return func(rule *PolicyRule, groupPeers []*nbpeer.Peer, direction int) {
			for _, peer := range groupPeers {
				...
				if protocol == PolicyRuleProtocolArtee VPNSSH {
...
func PeerSSHEnabledFromPolicies(policies []*Policy, peerID string, peerGroupIDs map[string]struct{}, peerSSHEnabled bool) bool {
	for _, policy := range policies {
		...
		for _, rule := range policy.Rules {
			...
			isSSHRule := rule.Protocol == PolicyRuleProtocolArtee VPNSSH ||
...
func expandPortsAndRanges(base FirewallRule, rule *PolicyRule, peer *nbpeer.Peer) []*FirewallRule {
	...
	if shouldCheckRulesForNativeSSH(features.nativeSSH, rule, peer) || rule.Protocol == PolicyRuleProtocolArtee VPNSSH {
