...
import (
	...
	"github.com/Artee VPNio/Artee VPN/management/server/integration_reference"
...
func assertGetAccountLoadsCustomDomains(t *testing.T, store Store) {
	...
	_, err := store.CreateCustomDomain(ctx, accountID, "example.com", "eu.proxy.Artee VPN.io", true)
	require.NoError(t, err, "creating the first custom domain must succeed")
	_, err = store.CreateCustomDomain(ctx, accountID, "apps.acme.io", "us.proxy.Artee VPN.io", false)
	...
	assert.Equal(t, "eu.proxy.Artee VPN.io", byDomain["example.com"], "custom domain must carry its target cluster")
	assert.Equal(t, "us.proxy.Artee VPN.io", byDomain["apps.acme.io"], "custom domain must carry its target cluster")
}
