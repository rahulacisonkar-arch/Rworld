...
func runTestForAllEngines(t *testing.T, testDataFile string, f func(t *testing.T, store Store)) {
	...
	for _, engine := range supportedEngines {
		if os.Getenv("Artee VPN_STORE_ENGINE") != "" && os.Getenv("Artee VPN_STORE_ENGINE") != string(engine) {
			continue
		}
		t.Setenv("Artee VPN_STORE_ENGINE", string(engine))
		...
		os.Unsetenv("Artee VPN_STORE_ENGINE")
	}
}
...
func TestSqlite_DeleteAccount(t *testing.T) {
	...
	t.Setenv("Artee VPN_STORE_ENGINE", string(types.SqliteStoreEngine))
...
func TestPostgresql_NewStore(t *testing.T) {
	...
	t.Setenv("Artee VPN_STORE_ENGINE", string(types.PostgresStoreEngine))
	store, cleanUp, err := NewTestStoreFromSQL(context.Background(), "", t.TempDir())
	t.Cleanup(cleanUp)
	assert.NoError(t, err)
...
func TestSqlStore_CreateGroup(t *testing.T) {
	...
	t.Setenv("Artee VPN_STORE_ENGINE", string(types.MysqlStoreEngine))
	store, cleanup, err := NewTestStoreFromSQL(context.Background(), "../testdata/extended-store.sql", t.TempDir())
	t.Cleanup(cleanup)
	require.NoError(t, err)
		...
		peers := []*nbpeer.Peer{
			{
				ID:        "peer1",
				AccountID: accountID,
				DNSLabel:  "peer1.Artee VPN.cloud",
				Key:       "peer1-key",
			...
			},
			{
				ID:        "peer2",
				AccountID: accountID,
				DNSLabel:  "peer2.Artee VPN.cloud",
				Key:       "peer2-key",
			...
			},
			{
				ID:        "peer3",
				AccountID: accountID,
				DNSLabel:  "peer3.Artee VPN.cloud",
				Key:       "peer3-key",
