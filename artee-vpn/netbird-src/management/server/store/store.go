...
import (
	...
	"github.com/Artee VPNio/Artee VPN/dns"
	...
	rpservice "github.com/Artee VPNio/Artee VPN/management/internals/modules/reverseproxy/service"
	...
	"github.com/Artee VPNio/Artee VPN/util/crypt"

	"github.com/Artee VPNio/Artee VPN/management/server/migration"
...
const (
	...
	postgresDsnEnvLegacy = "Artee VPN_STORE_ENGINE_POSTGRES_DSN"
	mysqlDsnEnv          = "NB_STORE_ENGINE_MYSQL_DSN"
	mysqlDsnEnvLegacy    = "Artee VPN_STORE_ENGINE_MYSQL_DSN"
)

// lookupDSNEnv checks the NB_ env var first, then falls back to the legacy Artee VPN_ env var.
...
func getStoreEngineFromEnv() types.Engine {
	// Artee VPN_STORE_ENGINE supposed to be used in tests. Otherwise, rely on the config file.
	kind, ok := os.LookupEnv("Artee VPN_STORE_ENGINE")
	if !ok {
		return ""
	}
	...
	if kind == types.FileStoreEngine {
		...
		if util.FileExists(storeFile) {
			return fmt.Errorf("%s is not supported. Please refer to the documentation for migrating to SQLite: "+
				"https://docs.Artee VPN.io/selfhosted/sqlite-store#migrating-from-json-store-to-sq-lite-store", types.FileStoreEngine)
		}
	}
	return nil
}
