# Artee VPN Management Server
Artee VPN management server will control and synchronize peers configuration within your Artee VPN account and network.

## Command Options
The CLI accepts the command **management** with the following options:
```shell
start Artee VPN Management Server

Usage:
  Artee VPN-mgmt management [flags]
...
Global Flags:
      --config string      Artee VPN config file location to write new config to (default "/etc/Artee VPN")
      --log-file string    sets Artee VPN log path. If console is specified the the log will be output to stdout (default "/var/log/Artee VPN/management.log")
...
```bash
# create a volume
docker volume create Artee VPN-mgmt
# run the docker container
docker run -d --name Artee VPN-management \
-p 33073:33073  \
-p 443:443  \
-v Artee VPN-mgmt:/var/lib/Artee VPN  \
-v ./config.json:/etc/Artee VPN/config.json  \
Artee VPNio/management:latest \
...
The certificate will be persisted in the ```datadir/letsencrypt/``` folder (e.g. ```/var/lib/Artee VPN/letsencrypt/```) inside the container.
...
```shell
docker volume inspect Artee VPN-mgmt
[
    {
...
```shell
shell $ docker run -d --name Artee VPN-management-debug \
-p 33073:33073  \
-v Artee VPN-mgmt:/var/lib/Artee VPN  \
-v ./config.json:/etc/Artee VPN/config.json  \
Artee VPNio/management:debug-latest

shell $ docker exec -ti Artee VPN-management-debug /bin/sh
container-shell $ 
```
