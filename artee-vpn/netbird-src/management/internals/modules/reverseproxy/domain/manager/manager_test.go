...
func TestGetClusterAllowList_BYOPMergedWithPublic(t *testing.T) {
	pm := &mockProxyManager{
		...
		getActiveClusterAddressesFunc: func(_ context.Context) ([]string, error) {
			return []string{"eu.proxy.Artee VPN.io"}, nil
		},
	}
	...
	assert.Equal(t, []string{"byop.example.com", "eu.proxy.Artee VPN.io"}, result)
}
...
func TestGetClusterAllowList_DeduplicatesBYOPAndPublic(t *testing.T) {
	pm := &mockProxyManager{
		...
		getActiveClusterAddressesFunc: func(_ context.Context) ([]string, error) {
			return []string{"shared.example.com", "eu.proxy.Artee VPN.io"}, nil
		},
	}
	...
	result, err := mgr.getClusterAllowList(context.Background(), "acc-123")
	require.NoError(t, err)
	assert.Equal(t, []string{"shared.example.com", "byop.example.com", "eu.proxy.Artee VPN.io"}, result)
}
...
func TestGetClusterAllowList_NoBYOP_FallbackToShared(t *testing.T) {
	pm := &mockProxyManager{
		...
		getActiveClusterAddressesFunc: func(_ context.Context) ([]string, error) {
			return []string{"eu.proxy.Artee VPN.io", "us.proxy.Artee VPN.io"}, nil
		},
	}
	...
	result, err := mgr.getClusterAllowList(context.Background(), "acc-123")
	require.NoError(t, err)
	assert.Equal(t, []string{"eu.proxy.Artee VPN.io", "us.proxy.Artee VPN.io"}, result)
}
...
func TestGetClusterAllowList_BYOPEmptySlice_FallbackToShared(t *testing.T) {
	...
	result, err := mgr.getClusterAllowList(context.Background(), "acc-123")
	require.NoError(t, err)
	assert.Equal(t, []string{"eu.proxy.Artee VPN.io"}, result)
}
