...
type TargetOptions struct {
	...
	// DirectUpstream bypasses the proxy's embedded Artee VPN client and dials
...
type Service struct {
	...
	// Private marks the service as Artee VPN-only: auth via ValidateTunnelPeer against AccessGroups instead of SSO. HTTP-only.
	Private bool
...
func (s *Service) validatePrivateRequirements() error {
	...
	if s.Auth.BearerAuth != nil && s.Auth.BearerAuth.Enabled {
		return errors.New("private services cannot enable bearer auth (SSO): Artee VPN-only access and SSO are mutually exclusive")
	}
	return nil
}
