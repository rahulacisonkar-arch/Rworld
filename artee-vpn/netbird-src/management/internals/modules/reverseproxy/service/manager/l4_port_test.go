...
func setupL4Test(t *testing.T, customPortsSupported *bool) (*Manager, store.Store, *proxy.MockController) {
	...
	mgr := &Manager{
		...
		clusterDeriver:     &testClusterDeriver{domains: []string{"test.Artee VPN.io"}},
	}
...
func TestCreateServiceFromPeer_TCP(t *testing.T) {
	...
	assert.Contains(t, resp.Domain, ".test.Artee VPN.io", "TCP uses unique subdomain")
...
func TestCreateServiceFromPeer_TLS(t *testing.T) {
	...
	assert.Contains(t, resp.Domain, ".test.Artee VPN.io", "TLS uses subdomain")
