...
import (
	...
	"github.com/Artee VPNio/Artee VPN/management/server/mock_server"
...
func setupIntegrationTest(t *testing.T) (*Manager, store.Store) {
	...
	mgr := &Manager{
		...
		clusterDeriver: &testClusterDeriver{
			domains: []string{"test.Artee VPN.io"},
		},
	}
...
func TestCreateServiceFromPeer(t *testing.T) {
	...
	t.Run("creates service with random domain", func(t *testing.T) {
		...
		assert.Contains(t, resp.Domain, "test.Artee VPN.io", "domain should use cluster domain")
...
func TestValidateSubdomainRequirement(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			domain:           "app.eu1.proxy.Artee VPN.io",
			cluster:          "eu1.proxy.Artee VPN.io",
			requireSubdomain: ptrBool(true),
			wantErr:          false,
		},
		{
			name:             "bare cluster domain, require_subdomain true",
			domain:           "eu1.proxy.Artee VPN.io",
			cluster:          "eu1.proxy.Artee VPN.io",
			requireSubdomain: ptrBool(true),
			wantErr:          true,
		},
...
// the embedded Artee VPN client and fail on every request.
