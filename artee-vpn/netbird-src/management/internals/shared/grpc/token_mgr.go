...
import (
	...
	authv2 "github.com/Artee VPNio/Artee VPN/shared/relay/auth/hmac/v2"
)
...
func (m *TimeBasedAuthSecretsManager) pushNewTURNAndRelayTokens(ctx context.Context, accountID, peerID string) {
	...
	update := &proto.SyncResponse{
		Artee VPNConfig: &proto.Artee VPNConfig{
			Turns: turns,
		},
	}
	...
	if m.relayCfg != nil {
		...
		if err == nil {
			update.Artee VPNConfig.Relay = &proto.RelayConfig{
	...
	m.extendArtee VPNConfig(ctx, peerID, accountID, update)
...
func (m *TimeBasedAuthSecretsManager) extendArtee VPNConfig(ctx context.Context, peerID, accountID string, update *proto.SyncResponse) {
	...
	extendedConfig := integrationsConfig.ExtendArtee VPNConfig(peerID, peerGroups, update.Artee VPNConfig, extraSettings)
	update.Artee VPNConfig = extendedConfig
}
