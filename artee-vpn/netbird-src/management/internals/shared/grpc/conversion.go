...
import (
	...
	nbversion "github.com/Artee VPNio/Artee VPN/version"
	log "github.com/sirupsen/logrus"
	goproto "google.golang.org/protobuf/proto"
	"google.golang.org/protobuf/types/known/timestamppb"

	integrationsConfig "github.com/Artee VPNio/management-integrations/integrations/config"

	"github.com/Artee VPNio/Artee VPN/client/ssh/auth"
	...
	"github.com/Artee VPNio/Artee VPN/shared/sshauth"
)

const (
	// deprecatedRemotePeersVersion is the version of Artee VPN that introduced the NetworkMap.RemotePeers field, deprecated in favor of RemotePeers.
...
func toArtee VPNConfig(config *nbconfig.Config, turnCredentials *Token, relayToken *Token, extraSettings *types.ExtraSettings) *proto.Artee VPNConfig {
	if config == nil {
		return nil
	}
	...
	nbConfig := &proto.Artee VPNConfig{
		Stuns:  stuns,
		Turns:  turns,
		Signal: signalCfg,
		Relay:  relayCfg,
...
func ToSyncResponse(ctx context.Context, config *nbconfig.Config, httpConfig *nbconfig.HttpServerConfig, deviceFlowConfig *nbconfig.DeviceAuthorizationFlow, peer *nbpeer.Peer, turnCredentials *Token, relayCredentials *Token, networkMap *types.NetworkMap, dnsName string, checks []*posture.Checks, dnsCache *cache.DNSConfigCache, settings *types.Settings, extraSettings *types.ExtraSettings, peerGroups []string, dnsFwdPort int64) *proto.SyncResponse {
	...
	nbConfig := toArtee VPNConfig(config, turnCredentials, relayCredentials, extraSettings)
	extendedConfig := integrationsConfig.ExtendArtee VPNConfig(peer.ID, peerGroups, nbConfig, extraSettings)
	response.Artee VPNConfig = extendedConfig
