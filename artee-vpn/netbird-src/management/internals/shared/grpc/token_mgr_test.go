...
var TurnTestHost = &config.Host{
	Proto:    config.UDP,
	URI:      "turn:turn.Artee VPN.io:77777",
	Username: "username",
	Password: "",
}
...
loop:
	...
	for _, update := range updates {
		if turns := update.Update.GetArtee VPNConfig().GetTurns(); len(turns) > 0 {
			turnUpdates++
			if turnUpdates == 1 {
		...
		if relay := update.Update.GetArtee VPNConfig().GetRelay(); relay != nil {
			// avoid updating on turn updates since they also send relay credentials
			if update.Update.GetArtee VPNConfig().GetTurns() == nil {
				relayUpdates++
				if relayUpdates == 1 {
