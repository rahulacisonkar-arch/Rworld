...
// --private alone wires the manager into the Artee VPN transport, so
...
func TestServer_PrivateInbound_Enabled_WiresLifecycle(t *testing.T) {
	...
	// Construct a Artee VPN transport. We can't actually start the embedded
	// client here (that needs a real management server), but we can
	// confirm that the lifecycle callbacks are registered.
	s.Artee VPN = roundtrip.NewArtee VPN(t.Context(), "test", "test", roundtrip.ClientConfig{
...
func TestWithTunnelLookup_AttachesLookupToContext(t *testing.T) {
	expected := auth.PeerIdentity{TunnelIP: netip.MustParseAddr("100.64.0.10"), FQDN: "peer.Artee VPN"}
