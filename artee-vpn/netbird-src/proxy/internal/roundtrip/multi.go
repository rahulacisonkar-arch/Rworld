...
// MultiTransport dispatches each request to either the embedded Artee VPN
...
// embedded Artee VPN path — current behaviour, preserved.
//
...
// what private (`Artee VPN proxy`) deployments and centralised proxies
...
// NewMultiTransport wires both branches. embedded is the existing Artee VPN
...
func NewMultiTransport(embedded http.RoundTripper, logger *log.Logger) *MultiTransport {
	...
	insecure.TLSClientConfig = &tls.Config{InsecureSkipVerify: true} //nolint:gosec // matches the embedded Artee VPN transport's per-target opt-in
...
// goes through the embedded Artee VPN roundtripper.
