...
// NewReverseProxy configures a new Artee VPN ReverseProxy.
...
func classifyProxyError(err error) (title, message string, code int, status web.ErrorStatus) {
	...
	case errors.Is(err, roundtrip.ErrNoPeerConnection),
		...
		return "Proxy Not Connected",
			"The proxy is not connected to the Artee VPN network. Please try again later or contact your administrator.",
			http.StatusBadGateway,
			web.ErrorStatus{Proxy: false, Destination: false}
	...
	case isHostUnreachable(err):
		return "Peer Not Connected",
			"The connection to the peer could not be established. Please ensure the peer is running and connected to the Artee VPN network.",
			http.StatusBadGateway,
			web.ErrorStatus{Proxy: true, Destination: false}
	}
...
const (
	// headerArtee VPNUser carries the authenticated user's display identity
	...
	headerArtee VPNUser = "X-Artee VPN-User"
	// headerArtee VPNGroups carries the user's group display names as a
	...
	headerArtee VPNGroups = "X-Artee VPN-Groups"
)
...
// stampArtee VPNIdentity injects authenticated identity onto outbound
// requests as X-Artee VPN-User and X-Artee VPN-Groups. Always strips any
// client-sent values first (anti-spoof). Skips when the request didn't
// carry CapturedData (early-path errors, internal endpoints).
func stampArtee VPNIdentity(r *httputil.ProxyRequest) {
	r.Out.Header.Del(headerArtee VPNUser)
	r.Out.Header.Del(headerArtee VPNGroups)
	...
	if email := cd.GetUserEmail(); isHeaderValueSafe(email) {
		r.Out.Header.Set(headerArtee VPNUser, email)
	}
	...
	if len(labels) > 0 {
		r.Out.Header.Set(headerArtee VPNGroups, strings.Join(labels, ","))
	}
}
