...
import (
	...
	"github.com/Artee VPNio/Artee VPN/proxy/auth"
	"github.com/Artee VPNio/Artee VPN/proxy/internal/roundtrip"
	"github.com/Artee VPNio/Artee VPN/proxy/internal/types"
	"github.com/Artee VPNio/Artee VPN/proxy/web"
)
...
func TestStampArtee VPNIdentity_NoCapturedData_StripsOnly(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)

	pr := newProxyRequest(t, "http://example.com/", "203.0.113.50:9999")
	pr.In.Header.Set(headerArtee VPNUser, "spoofed@evil.io")
	pr.In.Header.Set(headerArtee VPNGroups, "admin")
	pr.Out.Header = pr.In.Header.Clone()

	rewrite(pr)

	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNUser),
		"client-supplied X-Artee VPN-User must be stripped when no captured identity is present")
	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNGroups),
		"client-supplied X-Artee VPN-Groups must be stripped when no captured identity is present")
}

func TestStampArtee VPNIdentity_StampsFromCapturedData(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	cd.SetUserEmail("alice@Artee VPN.io")
	...
	assert.Equal(t, "alice@Artee VPN.io", pr.Out.Header.Get(headerArtee VPNUser),
		"captured email must overwrite any spoofed value")
	assert.Equal(t, "engineering,operations", pr.Out.Header.Get(headerArtee VPNGroups),
		"group display names must be CSV-joined in positional order")
}

// TestStampArtee VPNIdentity_GroupsOnlyWhenEmailEmpty covers the
...
// authorise, but X-Artee VPN-User stays unset — only its inbound stripping
// must happen.
func TestStampArtee VPNIdentity_GroupsOnlyWhenEmailEmpty(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNUser),
		"X-Artee VPN-User must remain unset when CapturedData carries no email")
	assert.Equal(t, "machines", pr.Out.Header.Get(headerArtee VPNGroups),
		"groups must still be stamped for peers without a user identity")
}

// TestStampArtee VPNIdentity_EmailOnlyWhenGroupsEmpty covers the symmetric
// case: identity-resolved user without resolved group memberships.
func TestStampArtee VPNIdentity_EmailOnlyWhenGroupsEmpty(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)

	pr := newProxyRequest(t, "http://example.com/", "203.0.113.50:9999")
	pr.In.Header.Set(headerArtee VPNGroups, "spoofed-admin")
	pr.Out.Header = pr.In.Header.Clone()

	cd := NewCapturedData("req-1")
	cd.SetUserEmail("carol@Artee VPN.io")

	pr.In = pr.In.WithContext(WithCapturedData(pr.In.Context(), cd))

	rewrite(pr)

	assert.Equal(t, "carol@Artee VPN.io", pr.Out.Header.Get(headerArtee VPNUser),
		"email must be stamped even when no groups are captured")
	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNGroups),
		"X-Artee VPN-Groups must remain unset when CapturedData carries no groups")
}

func TestStampArtee VPNIdentity_FallsBackToGroupIDsWhenNameMissing(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	cd := NewCapturedData("req-1")
	cd.SetUserEmail("bob@Artee VPN.io")
	...
	assert.Equal(t, "alpha,grp-b,grp-c", pr.Out.Header.Get(headerArtee VPNGroups),
		"empty-string and out-of-range name slots must both fall back to the group id")
}

// TestStampArtee VPNIdentity_DropsLabelsWithComma covers the
...
func TestStampArtee VPNIdentity_DropsLabelsWithComma(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
...
// TestStampArtee VPNIdentity_RejectsControlCharsInEmail covers the
...
func TestStampArtee VPNIdentity_RejectsControlCharsInEmail(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	assert.Equal(t, "engineering", pr.Out.Header.Get(headerArtee VPNGroups),
		"groups remain stampable even when email is invalid")
}

// TestStampArtee VPNIdentity_RejectsControlCharsInGroup covers the
// per-label defence: a group name with a control char is silently
// dropped, the rest are stamped.
func TestStampArtee VPNIdentity_RejectsControlCharsInGroup(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	assert.Equal(t, "operations", pr.Out.Header.Get(headerArtee VPNGroups),
		"group label with control char must be dropped, valid ones kept")
}

// TestStampArtee VPNIdentity_OmitsGroupsHeaderWhenAllInvalid covers the
// edge case where every group label is rejected: the header must not be
// set at all (rather than set to an empty string).
func TestStampArtee VPNIdentity_OmitsGroupsHeaderWhenAllInvalid(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	_, present := pr.Out.Header[http.CanonicalHeaderKey(headerArtee VPNGroups)]
	assert.False(t, present,
		"X-Artee VPN-Groups must not be set when every group label is rejected")
}
...
// TestStampArtee VPNIdentity_CapturedDataPresentButEmpty covers requests
...
func TestStampArtee VPNIdentity_CapturedDataPresentButEmpty(t *testing.T) {
	target, _ := url.Parse("http://backend.internal:8080")
	p := &ReverseProxy{forwardedProto: "auto"}
	rewrite := p.rewriteFunc(target, "", false, PathRewriteDefault, nil, nil)
	...
	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNUser),
		"X-Artee VPN-User must be stripped when CapturedData has no email")
	assert.Empty(t, pr.Out.Header.Get(headerArtee VPNGroups),
		"X-Artee VPN-Groups must be stripped when CapturedData has no groups")
}
