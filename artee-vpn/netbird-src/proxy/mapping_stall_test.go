...
func TestMappingStream_StallsWhenApplyBlocks(t *testing.T) {
	...
	// Artee VPN.AddPeer -> createClientEntry -> CreateProxyPeer, which blocks.
...
// embedded client via Artee VPN.RemovePeer -> client.Stop -> Engine.Stop, whose
...
func TestMappingStream_StallsWhenRemoveBlocks(t *testing.T) {
	...
	s := &Server{
		...
		routerReady:  closedChan(),
		lastMappings: make(map[types.ServiceID]*proto.ProxyMapping),
		// Stand in for Artee VPN.RemovePeer -> client.Stop hanging on
