...
// external IdP format to the embedded Dex IdP format used by Artee VPN >= v0.62.0.
//
...
// Usage:
//
//	Artee VPN-idp-migrate --config /etc/Artee VPN/management.json [--dry-run] [--force]
package main
...
import (
	...
	activitystore "github.com/Artee VPNio/Artee VPN/management/server/activity/store"
...
func formatSchemaErrors(errs []migration.SchemaError) string {
	...
	b.WriteString("\nPlease start the Artee VPN management server (v0.66.4+) at least once so that automatic database migrations create the required schema, then re-run this tool.\n")
	return b.String()
}
...
func printPostMigrationInstructions(cfg *migrationConfig) {
	...
	log.Info("Congratulations! You have successfully migrated your Artee VPN management server to the embedded Dex IdP.")
	log.Info("Next steps:")
