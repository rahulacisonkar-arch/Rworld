...
func configFromArgs(args []string) (*migrationConfig, error) {
	...
	fs := flag.NewFlagSet("Artee VPN-idp-migrate", flag.ContinueOnError)
...
//	Artee VPN_DOMAIN env (overrides flags, sets both)
//	--api-domain / --dashboard-domain flags (more specific than --domain)
//	Artee VPN_API_URL / Artee VPN_DASHBOARD_URL env (most specific, always wins)
//
...
func applyOverrides(cfg *migrationConfig, domain string) {
	...
	if val, ok := os.LookupEnv("Artee VPN_DOMAIN"); ok {
		cfg.dashboardURL = val
		cfg.apiURL = val
	}

	if val, ok := os.LookupEnv("Artee VPN_API_URL"); ok {
		cfg.apiURL = val
	}

	if val, ok := os.LookupEnv("Artee VPN_DASHBOARD_URL"); ok {
		cfg.dashboardURL = val
	}

	if val, ok := os.LookupEnv("Artee VPN_CONFIG_PATH"); ok {
		cfg.configPath = val
	}

	if val, ok := os.LookupEnv("Artee VPN_DATA_DIR"); ok {
		cfg.dataDir = val
	}

	if val, ok := os.LookupEnv("Artee VPN_IDP_SEED_INFO"); ok {
		cfg.idpSeedInfo = val
	}

	// Enforce dry run if any value is provided
	if sval, ok := os.LookupEnv("Artee VPN_DRY_RUN"); ok {
		if val, err := strconv.ParseBool(sval); err == nil {
			cfg.dryRun = val
		}
	}

	cfg.dryRun = parseBool("Artee VPN_DRY_RUN", cfg.dryRun)
	cfg.force = parseBool("Artee VPN_FORCE", cfg.force)
	cfg.skipConfig = parseBool("Artee VPN_SKIP_CONFIG", cfg.skipConfig)
	cfg.skipPopulateUserInfo = parseBool("Artee VPN_SKIP_POPULATE_USER_INFO", cfg.skipPopulateUserInfo)

	if val, ok := os.LookupEnv("Artee VPN_LOG_LEVEL"); ok {
		cfg.logLevel = val
	}
}
