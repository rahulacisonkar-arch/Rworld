...
This tool migrates Artee VPN deployments from an external IdP (Auth0, Zitadel, Okta, etc.) to the embedded Dex IdP introduced in v0.62.0. It does two things:
...
The tool is included in `.goreleaser.yaml` as the `Artee VPN-idp-migrate` build target. Each Artee VPN release produces pre-built archives for Linux (amd64, arm64, arm) that are uploaded to GitHub Releases. The archive naming convention is:

```
Artee VPN-idp-migrate_<version>_linux_<arch>.tar.gz
```

The build requires `CGO_ENABLED=1` because it links the SQLite driver used by `SqlStore`. The cross-compilation setup (CC env for arm64/arm) mirrors the `Artee VPN-mgmt` build.
...
| Env Var | Overrides |
|---------|-----------|
| `Artee VPN_DOMAIN` | Sets both `--dashboard-domain` and `--api-domain` |
| `Artee VPN_API_URL` | `--api-domain` |
| `Artee VPN_DASHBOARD_URL` | `--dashboard-domain` |
| `Artee VPN_CONFIG_PATH` | `--config` |
| `Artee VPN_DATA_DIR` | `--datadir` |
| `Artee VPN_IDP_SEED_INFO` | `--idp-seed-info` |
| `Artee VPN_DRY_RUN` | `--dry-run` (set to `"true"`) |
| `Artee VPN_FORCE` | `--force` (set to `"true"`) |
| `Artee VPN_SKIP_CONFIG` | `--skip-config` (set to `"true"`) |
| `Artee VPN_SKIP_POPULATE_USER_INFO` | `--skip-populate-user-info` (set to `"true"`) |
| `Artee VPN_LOG_LEVEL` | `--log-level` |

Resolution order: CLI flags are parsed first, then `--domain` sets both URLs, then `Artee VPN_DOMAIN` overrides both, then `Artee VPN_API_URL` / `Artee VPN_DASHBOARD_URL` override individually. After all resolution, `validateConfig()` ensures all required fields are set.
...
`decodeConnectorConfig()` base64-decodes and JSON-unmarshals the connector JSON provided via `--idp-seed-info` (or `Artee VPN_IDP_SEED_INFO`). It validates that the connector ID is non-empty. There is no auto-detection or fallback — the operator must provide the full connector configuration.
...
6. In `.goreleaser.yaml`:
   - Remove the `Artee VPN-idp-migrate` build entry
   - Remove the `Artee VPN-idp-migrate` archive entry
7. Run `go mod tidy`
