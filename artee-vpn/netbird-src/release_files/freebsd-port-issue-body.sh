#!/bin/bash
#
# FreeBSD Port Issue Body Generator for Artee VPN
#
...
#   - Fetch NEW version from latest Artee VPN GitHub release tag

set -e

GITHUB_REPO="Artee VPNio/Artee VPN"
PORTS_CGIT_URL="https://cgit.freebsd.org/ports/plain/security/Artee VPN/Makefile"
...
OUTPUT_FILE="${OUTPUT_DIR}/Artee VPN-${NEW_VERSION}-issue.txt"
...
Summary: security/Artee VPN: Update to ${NEW_VERSION}

Description:
------------
security/Artee VPN: Update ${OLD_VERSION} => ${NEW_VERSION}
...
echo "   - Summary: security/Artee VPN: Update to ${NEW_VERSION}"
echo "   - Description: Copy content from ${OUTPUT_FILE}"
echo "4. Attach diff file: Artee VPN-${NEW_VERSION}.diff"
