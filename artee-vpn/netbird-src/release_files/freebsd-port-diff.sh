#!/bin/bash
#
# FreeBSD Port Diff Generator for Artee VPN
#
...
GITHUB_REPO="Artee VPNio/Artee VPN"
PORTS_CGIT_BASE="https://cgit.freebsd.org/ports/plain/security/Artee VPN"
GO_PROXY="https://proxy.golang.org/github.com/Artee VPNio/Artee VPN/@v"
...
compute_checksums() {
    ...
    echo "SHA256 (go/security_Artee VPN/Artee VPN-v${version}/v${version}.mod) = ${mod_sha256}"
    echo "SIZE (go/security_Artee VPN/Artee VPN-v${version}/v${version}.mod) = ${mod_size}"
    echo "SHA256 (go/security_Artee VPN/Artee VPN-v${version}/v${version}.zip) = ${zip_sha256}"
    echo "SIZE (go/security_Artee VPN/Artee VPN-v${version}/v${version}.zip) = ${zip_size}"
    return 0
}
...
mkdir -p "${TMPDIR}/a/security/Artee VPN" "${TMPDIR}/b/security/Artee VPN"

echo "$OLD_MAKEFILE" > "${TMPDIR}/a/security/Artee VPN/Makefile"
echo "$OLD_DISTINFO" > "${TMPDIR}/a/security/Artee VPN/distinfo"
echo "$NEW_MAKEFILE" > "${TMPDIR}/b/security/Artee VPN/Makefile"
echo "$NEW_DISTINFO" > "${TMPDIR}/b/security/Artee VPN/distinfo"

# Generate diff
OUTPUT_FILE="${OUTPUT_DIR}/Artee VPN-${NEW_VERSION}.diff"

echo "" >&2
echo "Generating diff..." >&2

# Generate diff and clean up temp paths to show standard a/b paths
(cd "${TMPDIR}" && diff -ruN "a/security/Artee VPN" "b/security/Artee VPN") > "$OUTPUT_FILE" || true
