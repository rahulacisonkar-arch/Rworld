...
import (
	...
	"github.com/Artee VPNio/Artee VPN/shared/context"
)
...
func (hook ContextHook) parseSrc(filePath string) string {
	Artee VPNPath := strings.SplitAfter(filePath, hook.goModuleName)
	if len(Artee VPNPath) > 1 {
		return Artee VPNPath[len(Artee VPNPath)-1]
	}

	// in case of forked repo
	Artee VPNPath = strings.SplitAfter(filePath, "Artee VPN/")
	if len(Artee VPNPath) > 1 {
		return Artee VPNPath[len(Artee VPNPath)-1]
	}
