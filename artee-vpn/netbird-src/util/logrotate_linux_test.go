...
func TestFindFirstLogrotateConflict(t *testing.T) {
	t.Run("conflict in confDir", func(t *testing.T) {
		...
		conflictPath := filepath.Join(confDir, "Artee VPN")
		writeLogrotateConfig(t, conflictPath, `/var/log/Artee VPN/*.log {
    daily
    rotate 7
}`)
...
/var/log/Artee VPN/client.log { rotate 5 }`)
	...
	t.Run("no conflict when Artee VPN is absent", func(t *testing.T) {
		confPath, confDir := newLogrotateLayout(t)
		writeLogrotateConfig(t, filepath.Join(confDir, "nginx"), `/var/log/nginx/*.log { daily }`)
	...
	t.Run("commented-out Artee VPN line is ignored", func(t *testing.T) {
		confPath, confDir := newLogrotateLayout(t)
		writeLogrotateConfig(t, filepath.Join(confDir, "misc"), `# /var/log/Artee VPN/*.log { daily }
/var/log/other.log { weekly }`)
	...
	t.Run("subdirectories in confDir are ignored", func(t *testing.T) {
		...
		writeLogrotateConfig(t, filepath.Join(sub, "Artee VPN"), `/var/log/Artee VPN/*.log { daily }`)
