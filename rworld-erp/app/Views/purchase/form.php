...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1 class="page-title"><i class="fas fa-shopping-cart mr-2 text-success"></i>New Purchase Invoice</h1>
    </div>
...
<form method="POST" id="purForm">
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="qb-card mb-4">
                <div class="qb-card-header d-flex justify-content-between align-items-center">
                    <span class="qb-card-title">Purchase Items</span>
                    <div class="d-flex" style="gap:6px">
                        <input type="text" id="itemSearch" class="form-control form-control-sm" placeholder="Search item…" style="width:200px" autocomplete="off">
                        <button type="button" class="btn btn-sm btn-outline-success" onclick="addBlankRow()"><i class="fas fa-plus"></i> Row</button>
                    </div>
                </div>
                        ...
                        <table class="qb-table table-sm" id="lineTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-right" style="width:14%">Cost Price</th>
                            ...
                            <tfoot>
                                <tr><td colspan="4" class="text-right font-weight-bold">Subtotal</td><td class="text-right" id="subtotalDisp">$0.00</td><td></td></tr>
                                <tr>
                                ...
                                <tr class="font-weight-bold"><td colspan="4" class="text-right">Total</td><td class="text-right text-success" id="totalDisp">$0.00</td><td></td></tr>
                            </tfoot>
                        </table>
                    </div>
                </div>
        ...
        <div class="col-md-4">
            <div class="qb-card mb-4">
                <div class="qb-card-header"><span class="qb-card-title">Purchase Details</span></div>
                <div class="qb-card-body">
                    <div class="form-group">
                        <label class="form-label">Supplier</label>
                        <select name="supplier_id" class="form-control" onchange="setSupplierName()">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="supplier_inv_no" class="form-control" placeholder="Optional">
                    </div>
                    <div class="form-group">
            ...
            <button type="submit" class="btn btn-success btn-block" onclick="return prepareSubmit()">
...
<script>
const ITEMS = <?= json_encode(array_map(function($i){ return ['id'=>$i['id'],'description'=>$i['description'],'cost_price'=>(float)$i['cost_price'],'stock_no'=>$i['stock_no']]; }, $items)) ?>;
let lines = [];

function addBlankRow(item=null) {
    lines.push({ item_id:item?item.id:null, description:item?item.description:'', qty:1, unit_price:item?item.cost_price:0, discount_pct:0, amount:item?item.cost_price:0 });
    renderLines();
}
...
function renderLines() {
    ...
    lines.forEach((ln, i) => {
        ...
        tr.innerHTML = `<td><input type="text" class="form-control form-control-sm" value="${escHtml(ln.description)}" onchange="lines[${i}].description=this.value"></td>
...
function calcLine(i) {
    const ln = lines[i];
    ln.amount = Math.round(ln.qty * ln.unit_price * (1 - ln.discount_pct/100) * 100) / 100;
...
function setSupplierName() {
    const sel = document.getElementById('supplier_id') || document.querySelector('[name=supplier_id]');
    if (!sel) return;
    ...
    searchTimer = setTimeout(() => {
        fetch('<?= APP_URL ?>/item/search?q=' + encodeURIComponent(q))
            ...
            .then(matches => {
                ...
                dropdown.innerHTML = matches.map(i=>`<div class="p-2 border-bottom hover-bg" onclick="addBlankRow(${JSON.stringify({id:i.id,description:i.description,cost_price:parseFloat(i.cost_price),stock_no:i.stock_no}).replace(/"/g,'&quot;')});searchInput.value='';dropdown.style.display='none'" style="cursor:pointer">
                    <strong>${escHtml(i.description)}</strong> <small class="text-muted">${i.stock_no}</small>
                    <span class="float-right text-success">$${parseFloat(i.cost_price).toFixed(2)}</span></div>`).join('');
