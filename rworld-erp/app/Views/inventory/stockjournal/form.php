<!-- Stock Journal Create Form -->
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1 class="page-title"><i class="fas fa-exchange-alt mr-2 text-primary"></i>New Stock Adjustment</h1>
    </div>
    <a href="<?= APP_URL ?>/stockjournal" class="btn btn-outline-secondary"><i class="fas fa-arrow-left mr-1"></i> Back</a>
</div>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="qb-card mb-4" style="position:relative">
                <div class="qb-card-header d-flex justify-content-between align-items-center">
                    <span class="qb-card-title">Adjustment Items</span>
                ...
                <div class="qb-card-body p-0">
                    <table class="qb-table table-sm" id="lineTable">
                        ...
                        <tbody id="lineBody">
                            <tr id="emptyRow">
                                <td colspan="3" class="text-center py-4 text-muted">No items added — search and select an item above to add adjustment.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="col-md-4">
            <div class="qb-card mb-4">
                <div class="qb-card-header"><span class="qb-card-title">Adjustment Info</span></div>
                <div class="qb-card-body">
                    <div class="form-group">
                        <label class="form-label">Adjustment Direction</label>
                        <select name="direction" class="form-control">
                            <option value="in">Inbound (Stock In / Increase)</option>
                            <option value="out">Outbound (Stock Out / Waste / Decrease)</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Remarks / Reason</label>
                        <textarea name="remarks" class="form-control" rows="3" required placeholder="e.g. Broken stock, discrepancy audit…"></textarea>
                    </div>
                </div>
            </div>
            <button type="submit" class="btn btn-primary btn-block" onclick="return prepareSubmit()"><i class="fas fa-save mr-1"></i> Post Adjustment</button>
        </div>
    </div>
</form>
...
function addBlankRow(item=null) {
    if (!item) return;
    // Prevent duplicate entries of the same item
...
function renderLines() {
    ...
    if (lines.length === 0) {
        tbody.innerHTML = `<tr id="emptyRow"><td colspan="3" class="text-center py-4 text-muted">No items added — search and select an item above to add adjustment.</td></tr>`;
        return;
    }
