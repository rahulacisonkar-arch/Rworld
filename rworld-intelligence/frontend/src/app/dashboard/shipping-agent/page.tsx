...
export default function ShippingPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      // 1. Fetch AI jobs
      const jobsResp = await fetch("http://localhost:8000/api/shipping-agent/jobs", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (jobsResp.ok) setJobs(await jobsResp.json());

      // 2. Fetch Label Requests
      const reqsResp = await fetch("http://localhost:8000/api/shipping-agent/requests", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("rworld_token");
    fetch(`http://localhost:8000/api/shipping-agent/addresses?store_id=${selStoreId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((res) => res.json())
  ...
  const handleIngest = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("rworld_token");
    try {
      const response = await fetch("http://localhost:8000/api/shipping-agent/jobs/ingest", {
        method: "POST",
        headers: {
  ...
  const handleOCRSubmit = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("rworld_token");
    try {
      const response = await fetch("http://localhost:8000/api/shipping-agent/jobs/ocr", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleFetchQuotes = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/shipping-agent/rates?length=${pkgLen}&width=${pkgWid}&height=${pkgHgt}&weight_lbs=${pkgWgt}&ship_from_zip=${fromZip}&ship_to_zip=${toZip}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.ok) {
  ...
  const handleSubmitRequest = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/shipping-agent/requests", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleBuyLabel = async (reqId: number, quote: Quote) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/shipping-agent/requests/${reqId}/buy`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleCancelRequest = async (reqId: number) => {
    ...
    const token = localStorage.getItem("rworld_token");
    try {
      const response = await fetch(`http://localhost:8000/api/shipping-agent/requests/${reqId}/cancel`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
