  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const erpResponse = await fetch("/api/erp/reports/summary", {
        ...
        const shippingResponse = await fetch("/api/shipping-agent/jobs", {
          headers: { Authorization: `Bearer ${token}` },
        });
        ...
        const attResponse = await fetch("/api/attendance/employees", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (attResponse.ok) {
        ...
        const utilResponse = await fetch("/api/utility/bills", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (utilResponse.ok) {
