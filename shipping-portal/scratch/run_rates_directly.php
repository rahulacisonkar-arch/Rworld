...
if ($data && isset($data['rates'])) {
    echo "Found " . count($data['rates']) . " rates:\n";
    $count = 0;
    foreach ($data['rates'] as $r) {
        echo " - " . ($r['courier_name'] ?? 'N/A') . " (ID: " . ($r['courier_id'] ?? 'N/A') . "): $" . ($r['shipment_charge'] ?? $r['total_charge'] ?? '0.00') . " (Est: " . ($r['min_delivery_time'] ?? '') . "-" . ($r['max_delivery_time'] ?? '') . " days)\n";
        $count++;
        if ($count >= 10) break;
    }
} else {
    echo "Error response: " . $response1 . "\n";
}
