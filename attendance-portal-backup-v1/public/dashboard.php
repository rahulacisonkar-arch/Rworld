    ...
    if ($employeeExists) {
        if ($action === 'checkin') {
            ...
            if ($stmtCheckToday->fetchColumn()) {
                $_SESSION['action_error'] = "This employee has already checked in or completed their shift for today (only 1 check-in/out session is allowed per day).";
            } else {
                $stmtIn = $pdo->prepare("INSERT INTO attendance_logs (employee_id, store_id, login_time, date, status) VALUES (?, ?, ?, ?, 'Checked In')");
                $stmtIn->execute([$employeeId, $storeId, date('Y-m-d H:i:s'), date('Y-m-d')]);
                $_SESSION['action_success'] = "Employee checked in successfully.";
            }
            ...
            if ($logRow) {
                ...
                $stmtOut = $pdo->prepare("UPDATE attendance_logs SET logout_time = ?, status = 'Completed' WHERE id = ?");
                $stmtOut->execute([date('Y-m-d H:i:s'), $logId]);
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-end">
                                                    <div class="d-flex align-items-center justify-content-end gap-2">
                                                        ...
                                                        <?php else: ?>
                                                            <a href="dashboard.php?action=checkin&employee_id=<?php echo $emp['id']; ?>" class="btn btn-sm btn-primary px-3 font-semibold" style="border-radius: 6px; font-size: 0.78rem; background: var(--primary); border-color: var(--primary);">
