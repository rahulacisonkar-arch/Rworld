...
if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
    $w1_start = trim($_GET['start_date']);
    $w2_end = trim($_GET['end_date']);
    
    $totalDays = round((strtotime($w2_end) - strtotime($w1_start)) / 86400) + 1;
    if ($totalDays < 1) $totalDays = 1;
    $halfDays = ceil($totalDays / 2);
    
    $w1_end = date('Y-m-d', strtotime($w1_start . ' + ' . ($halfDays - 1) . ' days'));
    $w2_start = date('Y-m-d', strtotime($w1_end . ' + 1 day'));
    if ($w2_start > $w2_end) {
        $w2_start = $w2_end;
    }
} else {
        ...
        <div class="container-fluid p-4" style="margin-top: 20px;">
            ...
            <div class="card border-0 shadow-sm p-4 bg-white" style="border-radius: 12px;">
                <div class="pb-3 border-bottom mb-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <form action="payroll.php" method="GET" class="d-flex align-items-center gap-2 flex-wrap">
                            <label for="start_date" class="form-label mb-0 text-secondary" style="font-size: 0.8rem; white-space: nowrap;">From:</label>
                            <input type="date" name="start_date" id="start_date" class="form-control py-1 px-2.5" value="<?php echo e($w1_start); ?>" style="font-size: 0.82rem; border-radius: 6px; width: 140px;">
                            <label for="end_date" class="form-label mb-0 text-secondary" style="font-size: 0.8rem; white-space: nowrap;">To:</label>
                            <input type="date" name="end_date" id="end_date" class="form-control py-1 px-2.5" value="<?php echo e($w2_end); ?>" style="font-size: 0.82rem; border-radius: 6px; width: 140px;">
                            <button type="submit" class="btn btn-primary btn-sm px-3 py-1.5" style="border-radius: 6px; font-size: 0.82rem;">Filter</button>
                        </form>
                        <a href="export_payroll.php?start_date=<?php echo $w1_start; ?>&end_date=<?php echo $w2_end; ?>" class="btn btn-success font-semibold px-3 py-1.5" style="border-radius: 6px; font-size: 0.82rem;">
                            <i class="bi bi-file-earmark-excel me-1"></i>Download Excel
                        </a>
                    </div>
                </div>

                <!-- Thursday Pay Cycle Alert -->
                <div class="alert py-2.5 px-3.5 mb-3.5 d-flex align-items-center gap-2 border-0" style="font-size: 0.78rem; border-radius: 8px; background-color: rgba(75, 35, 8, 0.06); color: var(--primary);">
                    <i class="bi bi-info-circle-fill" style="font-size: 0.95rem;"></i>
                    <span><strong>Payment Schedule Note:</strong> Employees <strong>Crish Narveaz</strong> and <strong>MO</strong> are on a <strong>Thursday Payment Cycle</strong> (paid every Thursday).</span>
                </div>

                <form action="payroll.php?<?php echo isset($_GET['start_date']) ? 'start_date=' . urlencode($w1_start) . '&end_date=' . urlencode($w2_end) : 'week=' . urlencode($selectedDate); ?>" method="POST">
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div class="fw-semibold text-primary-dark" style="font-size: 0.88rem;">
                                                            <?php echo e($item['name']); ?>
                                                            <?php if (stripos($item['name'], 'Crish') !== false || strcasecmp($item['name'], 'Mo') === 0 || strcasecmp($item['name'], 'Mo Narveaz') === 0): ?>
                                                                <span class="badge text-white ms-1.5" style="background-color: var(--primary); font-size: 0.65rem; font-weight: 700; text-transform: uppercase; border-radius: 4px;">Thursday Pay</span>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                                </div>
