        ...
        if (!$emp) {
            ...
            try {
                ...
                if ($existing) {
                    ...
                    $success = "Future schedule successfully updated!";
                } else {
                    $stmtInsert = $pdo->prepare("INSERT INTO future_schedules (employee_id, store_id, date, scheduled_hours) VALUES (?, ?, ?, ?)");
                    $stmtInsert->execute([$employee_id, $storeId, $date, $scheduled_hours]);
                    $success = "Future schedule successfully added!";
                }
            } catch (Exception $e) {
        ...
        <div class="container-fluid p-4" style="margin-top: 20px;">
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4 space-y-6">
                    ...
                    <!-- Provide Future Schedule -->
                    <div class="card border-0 shadow-sm p-4 bg-white" style="border-radius: 12px;">
                        <h3 class="fw-bold mb-3 text-primary-dark border-bottom pb-2" style="font-size: 1.1rem; font-family: 'Outfit', sans-serif;">Provide Future Schedule</h3>
                        <form action="dashboard.php" method="POST" class="space-y-3">
                            <input type="hidden" name="action" value="add_schedule">
                            <?php csrf_input(); ?>

                            <div class="mb-3">
                                <label for="employee_id" class="form-label text-primary-dark fw-medium mb-1.5" style="font-size: 0.8rem;">Select Employee</label>
                                <select name="employee_id" id="employee_id" class="form-select" required style="border-radius: 8px; font-size: 0.85rem; padding: 8px 12px; border-color: var(--border);">
                            ...
                            <div class="mb-3">
                                <label for="schedule_date" class="form-label text-primary-dark fw-medium mb-1.5" style="font-size: 0.8rem;">Target Date</label>
                                <input type="date" name="date" id="schedule_date" class="form-control" value="<?php echo date('Y-m-d', strtotime('next monday')); ?>" required style="border-radius: 8px; font-size: 0.85rem; padding: 8px 12px; border-color: var(--border);">
                            </div>

                            <div class="mb-3">
                                <label for="scheduled_hours" class="form-label text-primary-dark fw-medium mb-1.5" style="font-size: 0.8rem;">Scheduled Hours (Weekly)</label>
                                <input type="number" step="0.01" name="scheduled_hours" id="scheduled_hours" class="form-control" placeholder="e.g. 40.00" required style="border-radius: 8px; font-size: 0.85rem; padding: 8px 12px; border-color: var(--border);">
                            </div>

                            <button type="submit" class="btn btn-primary w-100 font-semibold" style="background: var(--brand-brown); border-color: var(--brand-brown); border-radius: 8px; padding: 10px; font-size: 0.82rem;">
                                <i class="bi bi-calendar-plus me-1.5"></i>Submit Schedule
                            </button>
                        </form>
                    </div>
